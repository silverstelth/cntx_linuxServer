 ...
 * 
 *
 * $Id: logic_condition.cpp,v 1.1 2008/06/26 13:41:16 RoMyongGuk Exp $
 */
...
{ 
	...
	{
		sipwarning("(LOGIC)<CLogicComparisonBlock::setLogicStateMachine> The state machine is null");
	}
	else
	{
...
bool CLogicComparisonBlock::testLogic()
{
	...
	{
		sipwarning("(LOGIC)<CLogicComparisonBlock::testLogic> The variable %s is unknown in the state machine",VariableName.c_str());
		return false;
	}
	...
	sipwarning("(LOGIC)<CLogicComparisonBlock::testLogic> The comparison block operator %s is unknown",Operator.c_str());
	return false;
...
void CLogicComparisonBlock::write (xmlNodePtr node) const
{
	...
	xmlSetProp (elmPtr, (const xmlChar*)"Comparand", (const xmlChar*)toStringA(Comparand).c_str());
}
...
{ 
	if( logicStateMachine == 0 )
	{
		sipwarning("(LOGIC)<CCLogicConditionLogicBlock::setLogicStateMachine> The state machine is null");
	}
	else
	{
...
bool CLogicConditionLogicBlock::testLogic()
{
	switch( Type )
	{
		...
		case SUB_CONDITION :
		{
			...
			{
				sipwarning("(LOGIC)<CLogicConditionLogicBlock::testLogic> The subcondition \"%s\" is unknown in the state machine \"%s\"",
		...
		default :
			siperror("(LOGIC)<CLogicConditionLogicBlock::testLogic> logic block type %d is unknown",Type);
	}
...
void CLogicConditionLogicBlock::write (xmlNodePtr node) const
{
	...
	xmlSetProp (elmPtr, (const xmlChar*)"Type", (const xmlChar*)toStringA((uint32)Type).c_str());
	switch( Type )
	{
		case NOT : break;
...
{ 
	if( logicStateMachine == 0 )
	{
		sipwarning("(LOGIC)<CLogicConditionNode::setLogicStateMachine> The state machine is null");
	}
	else
	{
		// set the state machine of this node
...
{ 
	if( logicStateMachine == 0 )
	{
		sipwarning("(LOGIC)<CLogicCondition::setLogicStateMachine> The state machine is null");
	}
	else
	{
