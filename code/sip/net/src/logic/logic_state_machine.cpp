 ...
 * 
 *
 * $Id: logic_state_machine.cpp,v 1.1 2008/06/26 13:41:16 RoMyongGuk Exp $
 */
...
void xmlCheckNodeName (xmlNodePtr &node, const char *nodeName)
{
	...
	if ( node == NULL || ((const char*)node->name == NULL) || (strcmp ((const char*)node->name, nodeName) != 0) )
	{
		...
		if (node != NULL)
		{
			...
			if ( node != NULL && ((const char*)node->name != NULL) && (strcmp ((const char*)node->name, nodeName) == 0) )
			{
				sipinfo ("check node %s ok in the child", nodeName);
				return;
			}
		}
	...
	sipinfo ("check node %s ok", nodeName);
}
...
std::string getXMLProp (xmlNodePtr node, const char *propName)
{
	...
	if (name)
	{
		sipinfo ("get prop %s = %s", propName, name);
		string n = name;
		xmlFree ((void*)name);
		return n;
	}
...
void CLogicStateMachine::setCurrentState( string stateName )
{
	...
	if( itStates != _States.end() )
	{
		...
		sipinfo("Switching to state \"%s\"",_CurrentState.c_str());
	}
	else
	{
		sipwarning("(LOGIC)<CLogicStateMachine::setCurrentState> The state \"%s\" is not in the state machine \"%s\"",stateName.c_str(),_Name.c_str());
	}
...
{	
	...
	sipassert( itStates != _States.end() );
...
void CLogicStateMachine::modifyVariable( string varName, string modifOperator, sint64 value )
{
	...
	sipwarning("(LOGIC)<CLogicStateMachine::modifyVariable> The variable \"%s\" is not in the state machine \"%s\"",varName.c_str(),_Name.c_str());
...
void CLogicStateMachine::displayVariables()
{
	...
	*/
	...
	sipinfo("VARIABLES/COUNTERS in %s : %d/%d are registered : ",_Name.c_str(),allVariables.size(),allVariables.size()+unclaimedVariables.size());
	...
	for( itVars = allVariables.begin(); itVars != allVariables.end(); ++itVars )
	{
		...
		sipinfo("[%d] %s = %f",(uint8)(*itVars).first.getDynamicId(),(*itV).first.c_str(),(double)(*itV).second.getValue());
	}
	...
	for( itUV = unclaimedVariables.begin(); itUV != unclaimedVariables.end(); ++itUV )
	{
		...
		sipinfo("(-)%s = %f",(*itV).first.c_str(),(double)(*itV).second.getValue());
	}
...
{
	sipinfo("There are %d STATES in the state machine \"%s\": ",_States.size(),_Name.c_str());
	map<string, CLogicState>::const_iterator itS;
	for( itS = _States.begin(); itS != _States.end(); ++itS )
	{
		sipinfo("%s",(*itS).first.c_str());
	}
	sipinfo("The current state is : \"%s\"",_CurrentState.c_str());
...
void CLogicStateMachine::setVerbose( string varName, bool b )
{
	if( varName == "all" )
	{
		...
		if(b)
		{
			sipinfo("the verbose mode has been activated for all the variables",varName.c_str());
		}
		else
		{
			sipinfo("the verbose mode has been desactivated for all the variables",varName.c_str());
		}
		return;
	}
	...
	if(b)
	{
		sipinfo("the verbose mode for variable \"%s\" has been activated",varName.c_str());
	}
	else
	{
		sipinfo("the verbose mode for variable \"%s\" has been desactivated",varName.c_str());
	}
