...
#define LAYER3_SERVER_CLASS_NAME "L3Server"
	...
	public:
		...
		typedef std::map<TTcpSockId, CL3ServerRoute*>	TRouteMap;
		TRouteMap	_Routes;
		...
		virtual void update()
		{
			...
			for (; first != last; ++first)
			{
				...
				{
					sipdebug("NETL6:L3Server: sending KeepAlive message");
					// send a keep alive message
					CMessage keepAlive(M_KA);
		...
		void dump(SIPBASE::CLog &log) const
		{
			IModuleManager &mm = IModuleManager::getInstance();
			log.displayNL("  SIP Net layer 3 transport, SERVER mode");
			if (_CallbackServer.get() == NULL)
			{
...
#ifdef SIP_OS_UNIX
			unique_ptr<CCallbackServer> cbs = unique_ptr<CCallbackServer> (new CCallbackServer());
#else
		...
		void onConnection ( TTcpSockId from)
		{
			H_AUTO(L3S_onConnection);
			sipassert(_Routes.find(from) == _Routes.end());
		...
		void onDispatchMessage(const CMessage &msgin, TTcpSockId from, CCallbackNetBase &netbase)
		{
		...
		static void cbDispatchMessage (CMessage &msgin, TTcpSockId from, CCallbackNetBase &netbase)
		{
			// retrieve the transport instance
			TDispatcherIndex::iterator it(_DispatcherIndex.find(&netbase));
			sipassert(it != _DispatcherIndex.end());
	...
	SIPBASE_REGISTER_OBJECT(IGatewayTransport, CGatewayL3ServerTransport, std::string, string(LAYER3_SERVER_CLASS_NAME));

	void CL3ServerRoute::sendMessage(const CMessage &message) const
	{
		H_AUTO(L3SRoute_sendMessage);
		SIPNET_AUTO_DELTE_ASSERT;
...
#define LAYER3_CLIENT_CLASS_NAME "L3Client"
	...
	public:
		...
		typedef map<TTcpSockId, CL3ClientRoute*>	TClientRoutes;
		...
		typedef vector<TTcpSockId>		TClientRouteIds;
		...
		void deletePendingRoute()
		{
			...
			while (!_RouteToRemove.empty())
			{
				...
				_Routes.erase(route->CallbackClient.getTcpSockId());
			...
			for (; first != last; ++first)
			{
				...
				if (!route->CallbackClient.connected())
				{
					...
					if (route->LastConnectionRetry + _RetryInterval < now)
					{
						...
						try
						{
							sipdebug("Connecting to %s...", route->ServerAddr.asString().c_str());
							route->CallbackClient.connect(route->ServerAddr);
							sipdebug("Connected to %s", route->ServerAddr.asString().c_str());
							_Gateway->onRouteAdded(route);
						}
						catch(...)
						{
							sipinfo("Server %s still not available for connection", route->ServerAddr.asString().c_str());
						}
					}
				}
				else
				{
					...
					{
						sipdebug("NETL6:L3Client: sending KeepAlive message");
		...
		{
			IModuleManager &mm = IModuleManager::getInstance();
			log.displayNL("  SIP Net layer 3 transport, CLIENT mode");
		...
		bool onCommand(const TParsedCommandLine &command) throw (EInvalidCommand) 
		{
			...
			else if (commandName == "retryInterval")
			{
				...
				sipdebug("CGatewayL3ClientTransport : setting retry interval to %u", _RetryInterval);
			}
			else
				return false;
		...
		void connect(CInetAddress &addr)
		{
			...
			try
			{
				sipdebug("CGatewayL3ClientTransport : Connecting to %s...", addr.asString().c_str());
				route->LastConnectionRetry = CTime::getSecondsSince1970();
				// connect to the server
				route->CallbackClient.connect(addr);
				sipdebug("CGatewayL3ClientTransport : Connected to %s with connId %u", addr.asString().c_str(), connId);
			}
			catch (ESocketConnectionFailed e)
			{
				sipinfo("CGatewayL3ClientTransport : Failed to connect to server %s, retrying in %u seconds", addr.asString().c_str(), _RetryInterval);
			}

			// store the route
			_Routes.insert(make_pair(route->CallbackClient.getTcpSockId(), route.get()));
			_RouteIds[connId] = route->CallbackClient.getTcpSockId();
		...
		void close ( uint32 connId)
		{
			...
			{
				sipwarning("CGatewayL3ClientTransport : Invalid connectionId %u, max is %u", connId, _RouteIds.size()-1);
				return;
			}

			if (_RouteIds[connId] == NULL)
			{
				sipwarning("CGatewayL3ClientTransport : Invalid connectionId %u, the connection is unused now.", connId);
				return;		
			}
			...
			sipdebug("CGatewayL3ClientTransport : Closing connection %u to %s", connId, route->ServerAddr.asString().c_str());
		...
		void onDisconnection ( TTcpSockId from)
		{
			...
			sipdebug("CGatewayL3ClientTransport : Disconnection from %s", it->second->ServerAddr.asString().c_str());
...
//			_RouteToRemove.push_back(route);
	...
	// register this class in the transport factory
	SIPBASE_REGISTER_OBJECT(IGatewayTransport, CGatewayL3ClientTransport, std::string, string(LAYER3_CLIENT_CLASS_NAME));
