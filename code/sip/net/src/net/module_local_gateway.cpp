...
//		}
			...
			throw (EGatewayNotConnected)
		{
			...
			sipassert(_ModuleProxies.getB(moduleProxy) != NULL);
...
//			sipassert(  firstSource != lastSource );
//
...
//					_ThisProxy,
		...
		void				onModuleUnplugged(IModule *unpluggedModule)
		{
			...
			sipassert(it != _ModuleProxies.getBToAMap().end());
	...
	SIPNET_REGISTER_MODULE_FACTORY(CLocalGateway, "LocalGateway");
