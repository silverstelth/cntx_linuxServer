...
CInetAddress::CInetAddress( const std::basic_string<ucchar>& hostNameAndPort )
{
	ucstring	ucString = hostNameAndPort;
	std::string	strTemp = ucString.toString();
	init();
	setNameAndPort( strTemp );
}
...
void CInetAddress::setNameAndPort( const std::string& hostNameAndPort )
{
	...
	if (pos != string::npos)
	{
		uint16	uPort = atoi(hostNameAndPort.substr(pos + 1).c_str());
		setPort( uPort );
	}
	else
	{
...
CInetAddress& CInetAddress::setByName( const std::string& hostName )
{
...
#endif
	{
		...
		{
			_Valid = false;
			sipdebug( "LNETL0: Network error: resolution of hostname '%s' failed", hostName.c_str() );
			// return *this;
...
/*
 * Returns host name. (ex: "www.samilpo.org")
 */
...
//	return ss.str();
	return hostName() + ":" + SIPBASE::toStringA(port()) + " (" + ipAddress() + ")";
}
...
//	ss << ipAddress() << ":" << port();
//	return ss.str();
	return ipAddress() + ":" + SIPBASE::toStringA(port());
}

std::string CInetAddress::asUniqueString() const
{
	uint32 uIP = internalIPAddress();
	return SIPBASE::toStringA(uIP) + SIPBASE::toStringA(port()) + SIPBASE::toStringA(uIP);
}

/*
* Returns IP address and port as a string. (ex: "195.68.21.195:80")
*/
std::string CInetAddress::asWebIPString() const
{
	if(port() == 80)
		return ipAddress();
	//	stringstream ss;
 ...
 * Serialize
 */
void CInetAddress::serial( SIPBASE::IStream& s )
{
	SIPBASE::CMemStream *ms = dynamic_cast<SIPBASE::CMemStream*>(&s);
	if ( ms && ms->stringMode() )
	{
		// String stream
		string addrs;
...
CInetAddress CInetAddress::localHost()
{
	...
	CInetAddress localaddr;
	while (ifa_tmp) 
	{
		if ((ifa_tmp->ifa_addr) && (ifa_tmp->ifa_addr->sa_family == AF_INET))
		{
			localaddr.setSockAddr((struct sockaddr_in*)ifa_tmp->ifa_addr);
			if (localaddr.ipAddress() != "127.0.0.1") break;
		}
	...
	{
		sipwarning ("LNETL0: No network card detected! using localhost (127.0.0.1)");
	}
...
bool CInetAddress::isLocalHostIP(std::string strIP)
{
	if (strIP == "localhost")
		return true;
	...
	{
		if (strIP == hostaddrs[i].ipAddress())
			return true;
	}
	return false;
}
...
{
	string res;
	res = toStringA((addr)&0xFF);
	res += ".";
	res += toStringA((addr>>8)&0xFF);
	res += ".";
	res += toStringA((addr>>16)&0xFF);
	res += ".";
	res += toStringA((addr>>24)&0xFF);
	return res;
}
