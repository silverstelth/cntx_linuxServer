...
namespace SIPNET
{
	...
	{
		SIPBASE_SAFE_SINGLETON_DECL(CTransportIdAllocator);
	private:
	...
	public:
		...
		{
			sipassert(!_FreeIds.empty());
			TL5TransportId ret = _FreeIds.front();
			_FreeIds.pop_front();
			sipassert(_UsedIds.find(ret) == _UsedIds.end());
			_UsedIds.insert(ret);
			return ret;
		}
		...
		{
			sipassert(_UsedIds.find(id) != _UsedIds.end());
			_UsedIds.erase(id);
			_FreeIds.push_back(id);
		}
	};

	SIPBASE_SAFE_SINGLETON_IMPL(CTransportIdAllocator);
...
#define LAYER5_CLASS_NAME "L5Transport"
	...
	public:
		...
		~CGatewayL5Transport()
		{
			...
			sipassert(_TransportDispatcher.find(_TransportId) != _TransportDispatcher.end());
		...
		void dump(SIPBASE::CLog &log) const
		{
			...
			log.displayNL("  SIP Net layer 5 transport");
			if (!_Open)
			{
		...
		void open(const std::string &subNetName) throw (ETransportError)
		{
			...
			{
				{M_MODULE_15_TRANSPORT_GW_L5_MSG,	CGatewayL5Transport::cbDispatchL5Message	},
				{M_MODULE_15_TRANSPORT_GW_L5_ADDTP,	CGatewayL5Transport::cbL5AddTransport		},
				{M_MODULE_15_TRANSPORT_GW_L5_REMTP,	CGatewayL5Transport::cbL5RemoveTransport	},
			};
			...
			{
				sipdebug("LNETL6: L5 transport open : registering callbacks");
		...
		void onServiceUp(const std::string &serviceName, TServiceId sid)
		{
			...
			sipdebug("LNETL6: L5 transport onServiceUp('%s')", serviceName.c_str());
			...
			CMessage msg(M_MODULE_15_TRANSPORT_GW_L5_ADDTP);
			msg.serial(desc);
		...
		void onServiceDown(const std::string &serviceName, TServiceId sid)
		{
			...
			sipdebug("LNETL6: L5 transport onServiceDown('%hu')", sid.get());
			// retrieve the route
			TRouteMap::iterator it(_Routes.find(sid));
			if (it == _Routes.end())
			{
				sipwarning("Transport L5 : service down, can't find a route for the service");
				return;
			}
		...
		void onDispatchMessage(const CMessage &msgin, TServiceId sid)
		{
			...
			sipdebug("LNETL6: L5 transport onDispatchMessage from service %hu", sid.get());
			/// retrieve the route for dispatching
			TRouteMap::iterator it(_Routes.find(sid));
			if (it == _Routes.end())
			{
				sipwarning("Gateway '%s' : Can't find route for service %hu for dispatching, message is discarded", 
		...
		void onAddTransport(TServiceId sid, TTransportDesc &desc)
		{
			...
			sipdebug("LNETL6: L5 transport onAddTransport from service %hu", sid.get());
			...
			{
				sipdebug("LNETL6: L5 transport onAddTransport a route for this service alredy exist");
				return;
			}
		...
		void onRemoveTransport(TServiceId sid, TTransportDesc &desc)
		{
			...
			sipdebug("LNETL6: L5 transport onRemoveTransport from service %hu", sid.get());
			// Remove the route
			TRouteMap::iterator it(_Routes.find(sid));
			if (it == _Routes.end())
			{
				sipwarning("onRemoveTransport : can't find a route the the transport %hu on service %u", 
					desc.TransportId,
					sid.get());
				return;
			}
		...
		{
			sipdebug("LNETL6: L5 transport cbL5AddTransport from service %hu", sid.get());
		...
		{
			sipdebug("LNETL6: L5 transport cbL5RemoveTransport from service %hu", sid.get());
		...
		{
			sipdebug("LNETL6: L5 transport cbDispatch called, receiving from %s", serviceName.c_str());
			...
			{
				sipwarning("ReceiveL5Message, can't find transport id %u for dispatching, message is discarded",
					transportId);
				return;
			}
		...
		{
			sipdebug("LNETL6: L5 transport cbOnServiceUp called, service up for %s", serviceName.c_str());
		...
		{
			sipdebug("LNETL6: L5 transport cbOnServicedown called, service down for %s", serviceName.c_str());
	...
	SIPBASE_REGISTER_OBJECT(IGatewayTransport, CGatewayL5Transport, std::string, string(LAYER5_CLASS_NAME));
