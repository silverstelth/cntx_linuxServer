...
namespace SIPNET
{
			...
			else
			{
				...
				for (uint i=0; i<nbSecBlock; ++i)
				{
					...
					try
					{
						...
						sd = SIPBASE_GET_FACTORY(TSecurityData, uint8).createObject(dataTag, param);
					...
					catch(EStreamOverflow e)
					{
						...
						sipwarning("Error while reading stream for security data type %u", dataTag);

						s.seek(pos, SIPBASE::IStream::begin);
						sd = new TUnknownSecurityData(dataTag, blockSize);
						sd->serial(s);
					}
	...
	struct TModuleDescCodec
	{
		...
		void serial(SIPBASE::CMemStream &s)
		{
	...
	public:
		...
		virtual void createTransport(const std::string &transportClass, const std::string &instanceName)
		{
			...
			{
				sipwarning("A transport with the name '%s' already exist in this gateway", instanceName.c_str());
				return;
			}

			IGatewayTransport::TCtorParam param;
			param.Gateway = this;
			IGatewayTransport *transport = SIPBASE_GET_FACTORY(IGatewayTransport, std::string).createObject(transportClass, param);
			
			if (transport == NULL)
			{
				sipwarning("Failed to create a transport with the class '%s'", transportClass.c_str());
				return;
			}
...
//			_TransportPtrIdx.insert(make_pair(transport, ti));
			...
			sipdebug("NETL6: Gateway transport %s (%s) created", instanceName.c_str(), transportClass.c_str());
		}
		...
		virtual void deleteTransport(const std::string &instanceName)
		{
			...
			{
				sipwarning("Unknown transport named '%s'", instanceName.c_str());
				return;
			}
			
			sipdebug("NETL6: Gateway transport '%s' deleted", instanceName.c_str());
			// delete the transport
			IGatewayTransport *transport = it->second;
//			sipassert(_TransportPtrIdx.find(transport) != _TransportPtrIdx.end());
...
//			delete it->second;
		...
		virtual void	setTransportPeerInvisible(const std::string &transportInstanceName, bool peerInvisible)
		{
			...
			if (it == _Transports.end())
			{
				sipwarning("Unknown transport named '%s'", transportInstanceName.c_str());
				return;
			}
			...
			sipdebug("NETL6: Gateway transport %s peer invisible mode %s", transportInstanceName.c_str(), peerInvisible? "ON" : "OFF");
			...
			throw (EGatewayFirewallBreak)
		{
			...
			sipdebug("NETL6: Gateway transport %s firewall mode %s", transportInstanceName.c_str(), firewalled? "ON" : "OFF");
		...
		virtual void transportCommand(const TParsedCommandLine &commandLine)
		{
			for (uint i=1; i<commandLine.SubParams.size(); ++i)
			{
				...
				if (it == _Transports.end())
				{
					sipwarning("Unknown transport named '%s', ignoring command.", transportName.c_str());
				}
				else if (subParam->SubParams.empty())
				{
					sipwarning("Can't find sub param list for transport '%s' command", transportName.c_str());
				}
				else
				{
					sipdebug("NETL6: Gateway transport %s, sending command '%s'", transportName.c_str(), commandLine.toString().c_str());
		...
		{
			sipassert(route != NULL);
			// Remember the new route
			sipassert(_Routes.find(route) == _Routes.end());
...
//				sipassert(it != _ModuleProxies.end());
//
...
//			route->ForeignToLocalIdx.clear();
		...
		virtual void onReceiveMessage(CGatewayRoute *from, const CMessage &msgin)
		{
			...
			else if (msgin.getName() == M_MODULE_GATEWAY_MOD_OP)
			{
				onReceiveModuleMessageHeader(from, msgin);
			}
			else if (msgin.getName() == M_MODULE_GATEWAY_MOD_UPD)
			{
...
//			}
		...
		virtual void createSecurityPlugin(const std::string &className)
		{
			if (_SecurityPlugin != NULL)
			{
				sipwarning("SIPNETL5 : CStandardGateway::createSecurityPlugin : plug-in already created ");
				return;
			}

			CGatewaySecurity::TCtorParam params;
			params.Gateway = this;
			CGatewaySecurity *gs = SIPBASE_GET_FACTORY(CGatewaySecurity, std::string).createObject(className, params);
			if (gs == NULL)
			{
				sipwarning("SIPNETL5 : CStandardGateway::createSecurityPlugin : can't create a security plug-in for class '%s'", className.c_str());
				return;
			}
		...
		virtual void sendSecurityCommand(const TParsedCommandLine &command)
		{
			if (_SecurityPlugin != NULL)
			{
				sipwarning("SIPNETL5 : CStandardGateway::sendSecurityCommand : plug-in NOT created ");
				return;
			}
		...
		virtual void removeSecurityPlugin()
		{
			if (_SecurityPlugin == NULL)
			{
				sipwarning("SIPNETL5 : CStandardGateway::removeSecurityPlugin : plug-in not created");
				return;
			}
		...
		{
			sipassert(proxy->getModuleGateway() == this);
			sipassert(securityData->NextItem == NULL);

			CModuleProxy *modProx = dynamic_cast<CModuleProxy*>(proxy);
			sipassert(modProx != NULL);
		...
		void replaceAllSecurityDatas(IModuleProxy *proxy, TSecurityData *securityData)
		{
			...
			CModuleProxy *modProx = dynamic_cast<CModuleProxy*>(proxy);
			sipassert(modProx != NULL);
			sipassert(modProx->_SecurityData != securityData);
		...
		void onReceiveModuleMessage(CGatewayRoute *from, const CMessage &msgin)
		{
			...
			if (it == _ModuleProxies.end())
			{
				sipwarning("Can't dispatch the module message, sender proxy %u is not in this gateway", from->NextSenderProxyId);
				return;
			}
			...
			if (it == _ModuleProxies.end())
			{
				sipwarning("Can't dispatch the module message '%s', sender proxy %u is not in this gateway", msgin.toString().c_str(), from->NextAddresseeProxyId);
				return;
			}
		...
		void onReceiveModuleMessageHeader(CGatewayRoute *from, const CMessage &msgin)
		{
			...
			if (from->NextMessageType != CModuleMessageHeaderCodec::mt_invalid)
			{
				...
				sipwarning("Receiving a new module message header without having received the previous module message !");
			}
			...
			if (pmoduleId  == NULL)
			{
				sipwarning("The sender proxy %u is unknown in the translation table, can't dispatch the message !", from->NextSenderProxyId);
				from->NextMessageType = CModuleMessageHeaderCodec::mt_invalid;
				return;
			}
...
//				msgin.serialShortEnum(type);
				sipRead(msgin, serialShortEnum, type);
		...
		void onReceiveModuleAdd(CGatewayRoute *from, const CMessage &msgin)
		{
			...
			sipdebug("Gateway '%s' : store module info for '%s' (foreign ID %u) @ %u hop",
...
//					from->ForeignToLocalIdx.insert(make_pair(modDesc.ModuleProxyId, modProx->getModuleProxyId()));
				...
				if (modProx->getModuleDistance() > modInfo.ModuleDistance)
				{
					...
					sipdebug("Gateway '%s' : Use a shorter path for '%s' from %u to %u hops",
						getGatewayName().c_str(),
						modDesc.ModuleFullName.c_str(),
						proxy->_Distance,
...
//					from->ForeignToLocalIdx.insert(make_pair(modDesc.ModuleProxyId, modProx->getModuleProxyId()));
		...
		void onReceiveModuleDistanceUpdate(CGatewayRoute *from, const CMessage &msgin)
		{
			...
			sipRead(msgin, serial, moduleId);
			sipRead(msgin, serial, newDistance);

			// translate the module id
			const TModuleId *pModuleId = from->ForeignToLocalIdx.getB(moduleId);
			if (pModuleId == NULL)
			{
				sipwarning("Receive a module distance update for foreign module %u, but no translation available", moduleId);
				return;
			}
			...
			sipassert(it2 != _ModuleProxies.end());
			...
			for (; range.first != range.second; ++range.first)
			{
				...
				if (kmi.Route == from)
				{
					...
					sipdebug("Gateway '%s' : updating distance from %u to %u hop for module '%s'",
						getGatewayName().c_str(),
						kmi.ModuleDistance,
						newDistance,
			...
			sipassert(range.first != range.second);
			...
			if (proxy->_Route == from)
			{
				...
				if (proxy->_Distance < newDistance)
				{
					...
					for (; range.first != range.second; ++range.first)
					{
						...
						{
							sipdebug("Gateway '%s' : proxy '%s' use a new path from %u to %u hop",
								getGatewayName().c_str(),
					...
					if (range.first == range.second)
					{
						...
						sipdebug("Gateway '%s' : proxy '%s' path distance changed from %u to %u hop",
							getGatewayName().c_str(),
							proxy->getModuleName().c_str(),
							proxy->_Distance,
							newDistance);
				...
				else
				{
					...
					sipdebug("Gateway '%s' : proxy '%s' path distance reduced from %u to %u hop",
						getGatewayName().c_str(),
						proxy->getModuleName().c_str(),
						proxy->_Distance,
						newDistance);
...
//			msgin.serialPolyPtr(modSec);
			...
			if (pModuleId == NULL)
			{
				sipwarning("LNETL6 : receive module security update for unknown module foreign proxy %u", secChg.ModuleId);
				return;
			}
			...
			if (modProx == NULL)
			{
				sipwarning("LNETL6 : receive module security update for unknown module proxy %u, foreign %u", moduleId, secChg.ModuleId);
				return;
			}
		...
		{
			sipassert(moduleProxy->getModuleGateway() == this);
		...
		virtual void sendModuleMessage(IModuleProxy *senderProxy, IModuleProxy *addresseeProxy, const SIPNET::CMessage &message)
		{
			...
			{
				sipwarning("Module %u '%s' try to send message to %u '%s' but addressee is not visible, message discarded",
			...
			if (addresseeProxy->getGatewayRoute() == NULL)
			{
				...
				sipassert(senderProxy != NULL);
				sipassert(_ModuleProxies.find(senderProxy->getModuleProxyId()) != _ModuleProxies.end());
				...
				if (adrcp == NULL)
				{
					sipwarning("sendModuleMessage : can't find addressee module %u that is not plugged here !", addresseeModId);
					return;
				}
				...
				if (!addreseeMod->isImmediateDispatchingSupported())
				{
					...
					sipdebug("NETL6 : gateway '%s' : queuing local message '%s' from proxy %u to proxy %u",
						getModuleName().c_str(),
			...
			else
			{
				...
				CMessage msgHeader(M_MODULE_GATEWAY_MOD_OP);
		...
		virtual void dispatchModuleMessage(IModuleProxy *senderProxy, IModuleProxy *addresseeProxy, const CMessage &message)
		{
			...
			sipassert(addresseeProxy->getGatewayRoute() == NULL);
			...
			if (adrcp == NULL)
			{
				sipwarning("dispatchModuleMessage : dispatching a message to module %u that is not plugged here !", addresseeModId);
				return;
			}
...
//			addreseeMod->onProcessModuleMessage(senderProxy, message);
			...
			{
				sipwarning("An exception was thrown while dispatching message '%s' from '%s' to '%s'",
					message.toString().c_str(),
					senderProxy->getModuleName().c_str(),
					addresseeProxy->getModuleName().c_str());
		...
		void				onServiceUp(const std::string &serviceName, SIPNET::TServiceId serviceId)
		{
		}
		void				onServiceDown(const std::string &serviceName, SIPNET::TServiceId serviceId)
		{
		}
		void				onModuleUpdate()
		{
			...
			while (!_LocalMessages.empty())
			{
				...
				if (senderProx == NULL)
				{
					sipwarning("CStandardGateway : local message dispatching : Failed to retrieve proxy for sender module %u while dispatching message '%s' to %u", 
						lm.SenderProxyId,
						lm.Message.toString().c_str(),
						lm.AddresseProxyId);
				}
				else if (addresseeProx == NULL)
				{
					sipwarning("CStandardGateway : local message dispatching : Failed to retrieve proxy for addressee module %u while dispatching message '%s' from %u", 
						lm.AddresseProxyId,
						lm.Message.toString().c_str(),
						lm.SenderProxyId);
				}
		...
		bool				onProcessModuleMessage(IModuleProxy *senderModuleProxy, const CMessage &message)
		{
			...
			if (message.getName() == M_DEBUG_MOD_PING)
			{
				_PingCounter++;
		...
		void _sendModuleMessage(IModule *senderModule, TModuleId destModuleProxyId, const SIPNET::CMessage &message ) 
			throw (EModuleNotReachable, EModuleNotPluggedHere)
		{
			...
			sipassert(it != _LocalModuleIndex.end());
		...
		{
			sipdebug("NETL6: Gateway %s : plugging module '%s' id=%u",
				getModuleName().c_str(),
...
//
		...
		{
			sipdebug("NETL6: Gateway %s : unplugging module '%s' id=%u",
				getModuleName().c_str(),
			...
			for (; first != last; ++first)
			{
				...
				{
					// erase this message !
					sipwarning("CStandardGateway : while unplugging module %u from the gateway, locale message '%s' from proxy %u to proxy %u is lost",
						unpluggedModule->getModuleId(),
						lm.Message.toString().c_str(),
						lm.SenderProxyId,
						lm.AddresseProxyId);
		...
		void removeForeignModule(CGatewayRoute *route, TModuleId foreignModuleId)
		{
			...
			{
				// oups !
				sipwarning("removeForeignModule : unknown foreign module id %u", foreignModuleId);
				return;
			}
			...
			{
				// oups !
				sipwarning("Gateway '%s' : removeForeignModule : can't find proxy for id %u coming from foreign id %u", 
					getGatewayName().c_str(),
					proxyId, 
					foreignModuleId);
			...
			for (;range.first != range.second; ++range.first)
			{
				...
				if (kmi.Route == route)
				{
					sipdebug("Gateway '%s' : removing foreign module info for '%s'",
						getGatewayName().c_str(),
						CStringMapper::unmap(range.first->first).c_str());
...
//				sipassert(range.first != range.second);
			...
			if (range.first != range.second)
			{
				...
				sipassert(best != _KnownModules.end());
				...
				if (modProx->_Route != kmi.Route)
				{
					...
					sipdebug("Gateway '%s' : use a new route for module '%s' from %u to %u hop",
						getGatewayName().c_str(),
...
//			route->FirewallDisclosed.erase(proxy->getModuleProxyId());
		...
		void sendPendingModuleUpdate(CGatewayRoute *route)
		{
			...
			CMessage updateMsg(M_MODULE_GATEWAY_MOD_UPD);
...
//			}
		...
		SIPBASE_COMMAND_HANDLER_TABLE_EXTEND_BEGIN(CStandardGateway, CModuleBase)
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, dump, "dump various information about the gateway statue", "")
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, transportListAvailableClass, "list the available transport class", "no param")
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, transportAdd, "add a new transport to this gateway", "<transportClass> <instanceName>")
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, transportOptions, "set a gateway level option on a transport", "<transportClass> ( [PeerInvisible] [Firewalled] )")
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, transportCmd, "send a command to a transport", "[<transportName> ( <cmd specific to transport> )]*")
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, transportRemove, "remove an existing transport instance", "<transportName>")
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, securityListAvailableClass, "list the available security class", "no param")
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, securityCreate, "create a security plug-in", "<securityClassName>")
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, securityCommand, "send a command to the security plug-in", "<cmd specific to plug-in>")
			SIPBASE_COMMAND_HANDLER_ADD(CStandardGateway, securityRemove, "remove the security plug-in", "no parameter")
		SIPBASE_COMMAND_HANDLER_TABLE_END

		SIPBASE_CLASS_COMMAND_DECL(securityRemove)
		{
			if (_SecurityPlugin == NULL)
			{
		...
		SIPBASE_CLASS_COMMAND_DECL(securityCommand)
		{
		...
		SIPBASE_CLASS_COMMAND_DECL(securityCreate)
		{
			if (args.size() != 1)
				return false;
		...
		SIPBASE_CLASS_COMMAND_DECL(securityListAvailableClass)
		{
			if (args.size() != 0)
				return false;

			vector<string> list;
			SIPBASE_GET_FACTORY(CGatewaySecurity, std::string).fillFactoryList(list);
		...
		SIPBASE_CLASS_COMMAND_DECL(transportRemove)
		{
			if (args.size() != 1)
				return false;
		...
		SIPBASE_CLASS_COMMAND_DECL(transportCmd)
		{
		...
		SIPBASE_CLASS_COMMAND_DECL(transportOptions)
		{
			if (args.size() < 1)
				return false;
		...
		SIPBASE_CLASS_COMMAND_DECL(transportAdd)
		{
			if (args.size() != 2)
				return false;
		...
		SIPBASE_CLASS_COMMAND_DECL(transportListAvailableClass)
		{
			if (args.size() != 0)
				return false;

			vector<string> list;
			SIPBASE_GET_FACTORY(IGatewayTransport, std::string).fillFactoryList(list);
		...
		SIPBASE_CLASS_COMMAND_DECL(dump)
		{
			...
			SIPBASE_CLASS_COMMAND_CALL_BASE(CModuleBase, dump);
	...
	SIPNET_REGISTER_MODULE_FACTORY(CStandardGateway, "StandardGateway");
