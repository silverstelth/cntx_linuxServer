...
void			CHeapMemory::free(void *ptr)
{
	...
	{
		sipassert(itNext->second.Ptr >= (uint8*)ptr + size);
	}
	...
	if( itNext!= _EmptySpaces.begin() )
	{
		...
		sipassert(itPrevious!=_EmptySpaces.end());
		sipassert(itPrevious->second.Ptr + itPrevious->second.Size <= (uint8*)ptr );
	}
	else
