...
{
	return getMD5(SIPBASE::getUcsFromMbs(filename.c_str()));
}
...
CHashKeyMD5 getMD5(const std::basic_string<ucchar> &filename)
{
	...
	CIFile ifile;
	if (!ifile.open(CPath::lookup (filename, false)))
	{
		sipwarning (L"MD5: Can't open the file '%s'", filename.c_str());
...
static bool fromHex(char c, uint8 &x)
{
	...
	sipwarning("cannot convert to hexa");
	return false;
}
...
string CHashKeyMD5::toString() const
{
	string sTmp;
	for (uint32 i = 0; i < 16; ++i)
		sTmp += SIPBASE::toString("%02x", Data[i]);
	return sTmp;
}

basic_string<ucchar> CHashKeyMD5::toStringW() const
{
	basic_string<ucchar> sTmp;
	for (uint32 i = 0; i < 16; ++i)
#ifdef SIP_OS_WINDOWS
		sTmp += SIPBASE::toString(L"%02x", Data[i]);
#else
		sTmp += SIPBASE::toStringLW("%02x", Data[i]);
#endif
	return sTmp;
}
...
bool CHashKeyMD5::fromString(const std::basic_string<ucchar> &in)
{
	if (in.size() != 32)
	{
		sipwarning(L"bad string size");
		return false;
	}
...
// ****************************************************************************
void CHashKeyMD5::serial (SIPBASE::IStream &s)
{
