...
CHashKey getSHA1(const uint8 *buffer, uint32 size)
{
	...
	if (err)
	{
		sipwarning("SHA: SHA1Reset Error %d.\n", err );
		return CHashKey();
	}
	
	err = SHA1Input(&sha, (const unsigned char*)buffer, size);
	if (err)
	{
		sipwarning ("SHA: SHA1Input Error %d.\n", err);
		return CHashKey();
	}
	...
	if (err)
	{
		sipwarning("SHA: SHA1Result Error %d, could not compute message digest.\n", err );
		return CHashKey();
	}
...
CHashKey getSHA1(const string &filename)
{
	...
	{
		sipwarning ("SHA: Can't open the file '%s'", filename.c_str());
		return CHashKey();
	}
