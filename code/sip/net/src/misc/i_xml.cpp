...
#define SIPBASE_READ_BUFFER_SIZE 1024
...
bool CIXml::init (IStream &stream)
{
	...
	if (stream.isReading())
	{
		...
		// The read buffer
        char buffer[SIPBASE_READ_BUFFER_SIZE];
		...
		// For all the file
        while (length>=SIPBASE_READ_BUFFER_SIZE)
		{
			// Fill the buffer
			stream.serialBuffer ((uint8*)buffer, SIPBASE_READ_BUFFER_SIZE);

			// Read a buffer
            int res = xmlParseChunk(_Parser, buffer, SIPBASE_READ_BUFFER_SIZE, 0);
			...
			// Length left
			length -= SIPBASE_READ_BUFFER_SIZE;
        }
	...
	{
		sipwarning ("XML: The stream is not an input stream.");
	}
...
bool	CIXml::init (ucstring filename)
{
	CIFile	f;
	if ( !f.open(filename.c_str(), true) )
		return false;
	try
	{
		if (!init(f))
...
void CIXml::serialSeparatedBufferIn ( string &value, bool checkSeparator )
{
			...
			if (_PushBegin)
			{
				...
				if (_AttribPresent)
				{
					...
					else
					{
						...
						sipassert (_CurrentElement->name);

						// Make an error message
						char tmp[512];
						smprintf (tmp, 512, "SIP XML Syntax error in block line %d\nAttribute \"%s\" is missing in node \"%s\"", 
	...
	{
		siperror ( "Output stream has not been initialized" );
	}
}
...
bool CIXml::xmlPushBeginInternal (const char *nodeName)
{
	...
	else
	{
		...
		if ( _Parser )
		{
				...
				if (_CurrentNode==NULL)
				{
					...
					if (_CurrentNode)
					{
						...
						if ( (_CurrentNode->type != XML_ELEMENT_NODE) || ( (const char*)_CurrentNode->name != string(nodeName)) )
						{
							// Make an error message
							char tmp[512];
							smprintf (tmp, 512, "SIP XML Syntax error : root node has the wrong name : \"%s\" should have \"%s\"",
					...
					else
					{
						// Make an error message
						char tmp[512];
						smprintf (tmp, 512, "SIP XML Syntax error : no root node found.");
						throw EXmlParsingError (tmp);
					}
				}
			...
			{
				siperror ( "You must close your xmlPushBegin - xmlPushEnd before calling a new xmlPushBegin.");
				return false;
			}
		}
		else
...
bool CIXml::xmlPushEndInternal ()
{
	...
	else
	{
		...
		if ( _Parser )
		{
			...
			{
				siperror ( "You must call xmlPushBegin before calling xmlPushEnd.");
				return false;
			}
		}
		else
...
bool CIXml::xmlPopInternal ()
{
	...
	else
	{
		...
		if ( _Parser )
		{
			...
			{
				siperror ( "You must call xmlPop after xmlPushEnd.");
				return false;
			}
		}
		else
...
bool CIXml::xmlSetAttribInternal (const char *attribName)
{
	...
	else
	{
		...
		if ( _Parser )
		{
			...
			{
				siperror ( "You must call xmlSetAttrib between xmlPushBegin and xmlPushEnd calls.");
				return false;
			}
		}
		else
...
xmlNodePtr CIXml::getChildNodeOfIntProperty (xmlNodePtr parent, const char *childName, const char *property, int nVal)
{
	xmlNodePtr	pChild = CIXml::getFirstChildNode(parent, childName);
	xmlNodePtr	pTarget = NULL;
	while(pChild)
	{
		int	nID = CIXml::getIntProperty(pChild, property, -1);
		if (nID == nVal)
			return pChild;
		pChild = CIXml::getNextChildNode(pChild, childName);
	}
	return NULL;
}
...
xmlNodePtr CIXml::getChildNodeOfStringProperty (xmlNodePtr parent, const char *childName, const char *property, const char *value)
{
	xmlNodePtr	pChild = CIXml::getFirstChildNode(parent, childName);
	xmlNodePtr	pTarget = NULL;
	while(pChild)
	{
		string strV = CIXml::getStringProperty(pChild, property, "");
		if (strV == value)
			return pChild;
		pChild = CIXml::getNextChildNode(pChild, childName);
	}
...
int CIXml::getIntProperty(xmlNodePtr node, const char *property, int defaultValue)
{
	...
	b = getPropertyString(str, node, property);
	if (b == false)
		return defaultValue;
	...
	if (val==0 && s!= "0")
	{
		sipwarning("bad integer value: %s", s.c_str());
		return defaultValue;
	}
