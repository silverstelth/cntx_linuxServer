...
SIPBASE_SAFE_SINGLETON_IMPL(CEntityIdTranslator);
...
void CEntityIdTranslator::CEntity::serial (SIPBASE::IStream &s)
{
...
bool CEntityIdTranslator::checkEntityName (const ucstring &entityName )
{
	...
	if (!isValidEntityName (entityName,SIPBASE::InfoLog)) return false;
...
void CEntityIdTranslator::registerEntity (const CEntityId &eid, const ucstring &entityName, sint8 entitySlot, uint32 uid, const string &userName, uint32 shardId)
{
	...
	{
		sipwarning ("EIT: Can't register EId %s EntityName '%s' UId %d UserName '%s' because EId is already in the map", reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
		return;
	}
	...
	if (!checkEntityName(entityName))
	{
		if (isValidEntityName(entityName))
			sipwarning ("EIT: Can't register EId %s EntityName '%s' UId %d UserName '%s' because EntityName is already in the map", reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
		else
			sipwarning ("EIT: Can't register EId %s EntityName '%s' UId %d UserName '%s' because EntityName is invalid", reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
		return;
	}
	
	sipinfo ("EIT: Register EId %s EntityName '%s' UId %d UserName '%s'", reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
...
void CEntityIdTranslator::updateEntity (const CEntityId &eid, const ucstring &entityName, sint8 entitySlot, uint32 uid, const std::string &userName, uint32 shardId)
{
	...
	else
	{
		...
		if (entity.EntityName != entityName)
		{
			if (!checkEntityName(entityName))
			{
				sipwarning ("EIT: Can't update EId %s EntityName '%s' UId %d UserName '%s' with new name '%s' because EntityName is already in the map", 
					reid.toString().c_str(), 
...
void CEntityIdTranslator::unregisterEntity (const CEntityId &eid)
{
	...
	if (it == RegisteredEntities.end ())
	{
		sipwarning ("EIT: Can't unregister EId %s because EId is not in the map", reid.toString().c_str());
		return;
	}

	CEntity &entity = it->second;

	sipdebug ("EIT: Unregister EId %s EntityName '%s' UId %d UserName '%s'", reid.toString().c_str(), entity.EntityName.toString().c_str(), entity.UId, entity.UserName.c_str());
	NameIndex.erase(toLower(entity.EntityName));
...
void CEntityIdTranslator::checkEntity (const CEntityId &eid, const ucstring &entityName, uint32 uid, const string &userName)
{
	...
	sipinfo ("EIT: Checking EId %s EntityName '%s' UId %d UserName '%s'", reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
	
	if (it == RegisteredEntities.end ())
	{
		sipwarning ("EIT: Check failed because EId is not in the CEntityIdTranslator map for EId %s EntityName '%s' UId %d UserName '%s'", reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
		
		if (checkEntityName(entityName))
		{
			sipwarning ("EIT: Check failed because entity name already exist '%s' for EId %s EntityName '%s' UId %d UserName '%s'", getByEntity(entityName).toString().c_str(), reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
		}
	}
	else
	{
		...
		if (entity.EntityName != entityName)
		{
			sipwarning ("EIT: Check failed because entity name not identical '%s' in the CEntityIdTranslator map for EId %s EntityName '%s' UId %d UserName '%s'", entity.EntityName.toString().c_str(), reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
			if(!entityName.empty())
			{
				entity.EntityName = entityName;
			}
		...
		if (entity.UId != uid)
		{
			sipwarning ("EIT: Check failed because uid not identical (%d) in the CEntityIdTranslator map for EId %s EntityName '%s' UId %d UserName '%s'", entity.UId, reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
			if (uid != 0)
			{
				entity.UId = uid;
			}
		...
		{
			sipwarning ("EIT: Check failed because user name not identical '%s' in the CEntityIdTranslator map for EId %s EntityName '%s' UId %d UserName '%s'", entity.UserName.c_str(), reid.toString().c_str(), entityName.toString().c_str(), uid, userName.c_str());
			if(!userName.empty())
			{
				entity.UserName = userName;
			}
...
void cbInvalidEntityNamesFilename(const std::string &invalidEntityNamesFilename)
{
	...
	if (fn.empty())
	{
		sipwarning ("EIT: Can't load filename '%s' for invalid entity names filename (not found)", invalidEntityNamesFilename.c_str());
		return;
	}
	...
	if (fp == NULL)
	{
		sipwarning ("EIT: Can't load filename '%s' for invalid entity names filename", fn.c_str());
		return;
	}
...
void CEntityIdTranslator::load (const string &fileName, const string &invalidEntityNamesFilename)
{
	...
	if (fileName.empty())
	{
		sipwarning ("EIT: Can't load empty filename for EntityIdTranslator");
		return;
	}

	if (!FileName.empty())
	{
		sipwarning ("EIT: Can't load file '%s' for EntityIdTranslator because we already load the file '%s'", fileName.c_str(), FileName.c_str());
		return;
	}
	
	sipinfo ("EIT: CEntityIdTranslator: load '%s'", fileName.c_str());
	...
	if(CFileA::fileExists(FileName))
	{
		...
		{
			sipwarning ("EIT: Can't load filename '%s' for EntityIdTranslator", FileName.c_str());
		}
	}

	cbInvalidEntityNamesFilename (invalidEntityNamesFilename);

	SIPBASE::CFileA::addFileChangeCallback (invalidEntityNamesFilename, cbInvalidEntityNamesFilename);
}
...
void CEntityIdTranslator::save ()
{
	...
	if (FileName.empty())
	{
		sipwarning ("EIT: Can't save empty filename for EntityIdTranslator (you forgot to load() it before?)");
		return;
	}

	sipinfo ("EIT: CEntityIdTranslator: save");
	...
	{
		sipwarning ("EIT: Can't save filename '%s' for EntityIdTranslator", FileName.c_str());
	}
}
...
void CEntityIdTranslator::getEntityIdInfo (const CEntityId &eid, ucstring &entityName, sint8 &entitySlot, uint32 &uid, string &userName, bool &online, std::string* additional)
{
	...
	if (it == RegisteredEntities.end ())
	{
		sipwarning ("EIT: %s is not registered in CEntityIdTranslator", reid.toString().c_str());
		entityName = "";
		entitySlot = -1;
		uid = ~0;
		userName = "";
...
SIPBASE_CATEGORISED_COMMAND(sip, findEIdByUser,"Find entity ids using the user name","<username>|<uid>")
{
	if (args.size () != 1)
		return false;
...
SIPBASE_CATEGORISED_COMMAND(sip, findEIdByEntity,"Find entity id using the entity name","<entityname>|<eid>")
{
	if (args.size () != 1)
		return false;
...
SIPBASE_CATEGORISED_COMMAND(sip, entityNameValid,"Tell if an entity name is valid or not using CEntityIdTranslator validation rulez","<entityname>")
{
...
SIPBASE_CATEGORISED_COMMAND(sip, playerInfo,"Get informations about a player or all players in CEntityIdTranslator","[<entityname>|<eid>|<username>|<uid>]")
{
	if (args.size () == 0)
	{
