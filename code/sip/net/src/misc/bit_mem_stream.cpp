...
void	CBitMemStream::_serialAndLog( const char *argstr, uint32& value, uint nbits )
{
	...
	if ( VerboseAllTraffic )
		sipdebug( "TRAFFIC/%p/%s: %s: %u bits at bitpos %d (%u)", this, isReading()?"I":"O", argstr, nbits, bitpos, value );
}
...
void	CBitMemStream::_serialBitAndLog( const char *argstr, bool& bit )
{
	...
	serialBit( bit );
	if ( VerboseAllTraffic )
		sipdebug( "TRAFFIC/%p/%s: %s: 1 bit at bitpos %d (%hu)", this, isReading()?"I":"O", argstr, bitpos, (uint16)bit );
}

SIPBASE_CATEGORISED_COMMAND(sip, verboseAllTraffic, "Verbose the all-traffic logs", "" )
{
	if(args.size()>1)
		return false;
	...
	sipinfo("BMS: verboseAllTraffic is %s",VerboseAllTraffic?"ON":"OFF");
	return true;
}
...
void	CBitMemStream::internalSerial( uint32& value, uint nbits, bool resetvalue )
{
	if ( nbits == 0 )
		return;
	if ( nbits > 32 )
		throw EMemStream (string("trying to serial ")+SIPBASE::toStringA(nbits)+string(" bits"));
	...
	if ( isReading() )
	{
		...
		if ( nbits > _FreeBits )
		{
			//sipdebug( "Reading byte %u from %u free bits (%u remaining bits)", lengthS(), _FreeBits, nbits );
...
//			++_BufPos;
		...
		{
			//sipinfo( "Reading last byte %u from %u free bits (%u remaining bits)", lengthS(), _FreeBits, nbits );
...
//				++_BufPos;
	...
	else
	{
		...
		sipassert( ! ((_FreeBits == 8) && (*(buffer+_Buffer.Pos) != 0)) );
#endif
		...
		if ( nbits > _FreeBits )
		{
			...
			//sipdebug( "Writing byte %u into %u free bits (%u remaining bits)", lengthS(), _FreeBits, nbits );
			//displayDwordBits( value, 32, nbits-1, false );
//			*_BufPos |= (v >> (nbits - _FreeBits));
		...
		{
			// Shorter or equal
			//sipdebug( "Writing last byte %u into %u free bits (%u remaining bits)", lengthS(), _FreeBits, nbits );
...
#ifdef LOG_ALL_TRAFFIC
	if ( VerboseAllTraffic )
		sipdebug( "TRAFFIC/%p/%s: Reserving %u bits at bitpos %d", this, isReading()?"I":"O", nbits, getPosInBit() );	
#endif
...
//		sipassert( _BufPos < _Buffer.getPtr() + _Buffer.size() );
		sipassert( _Buffer.Pos < _Buffer.getBuffer().size() );
...
{
#ifdef SIP_DEBUG
	sipassert( (nbits <= 32) && (nbits != 0) );
	sipassert( ! isReading() );
	sipassert( bitpos+nbits <= (uint)getPosInBit() );
#endif
...
#ifdef SIP_DEBUG
	sipassert( ! isReading() );
	sipassert( bitpos+bitfield.size() <= (uint)getPosInBit() );
#endif
 ...
 */
void	CBitMemStream::readBits( SIPBASE::CBitSet& bitfield )
{
#ifdef SIP_DEBUG
	sipassert( isReading() );
#endif
	...
	if ( len != 0 )
	{
#ifdef LOG_ALL_TRAFFIC
		if ( VerboseAllTraffic )
			sipdebug( "TRAFFIC/%p/%s: Reading %u bits bitfield at bitpos %d", this, isReading()?"I":"O", len, getPosInBit() );
#endif
		uint i = 0;
		uint32 v;
		while ( len > 32 )
		{
			serial( v, 32 );
			//sipdebug( "Bitfield: Read %u at %d", v, _BufPos-_Buffer.getPtr()-4 );
...
void	CBitMemStream::serial(std::string &b) 
{
	...
	if ( isReading() )
	{
		...
		if (len > length()-(uint32)getPos())
			throw SIPBASE::EInvalidDataStream( "BMS: Trying to read a string of %u bytes, past stream size", len );
		b.resize( len );
	}
	else
	{
		len = b.size();
		if (len>1000000)
			throw SIPBASE::EInvalidDataStream( "BMS: Trying to write a string of %u bytes", len );
		serial( len );
	}
...
#ifdef LOG_ALL_TRAFFIC
	if ( VerboseAllTraffic )
		sipdebug( "TRAFFIC/%p/%s: String (size 32+%u*8 bits) at bitpos %d", this, isReading()?"I":"O", len, bitpos );
#endif
}
	...
	if ( _StringMode )
	{
		...
		if(isReading())
		{
			...
			if (len > (uint32)(sint32(length())-sint32(getPos())))
				throw SIPBASE::EInvalidDataStream( "BMS: Trying to read an ucstring of %u bytes, past stream size", len );
			b.resize(len);
		}
		else
		{
			len= b.size();
			if (len>1000000)
				throw SIPBASE::EInvalidDataStream( "BMS: Trying to write an ucstring of %u bytes", len );
			serial(len);
		}
...
#ifdef LOG_ALL_TRAFFIC
	if ( VerboseAllTraffic )
		sipdebug( "TRAFFIC/%p/%s: Ucstring at bitpos %d", this, isReading()?"I":"O", bitpos );
#endif
...
void	CBitMemStream::serialMemStream(CBitMemStream &b)
{
	...
	if ( isReading() )
	{
		...
		if (len > length()-getPos())
			throw SIPBASE::EInvalidDataStream( "BMS: Trying to read a BMS of %u bytes, past stream size", len );
...
#ifdef LOG_ALL_TRAFFIC
	if ( VerboseAllTraffic )
		sipdebug( "TRAFFIC/%p/%s: Sub-bitmemstream (size 32+%u*8 bits) at bitpos %d", this, isReading()?"I":"O", len, bitpos );
#endif
}
...
void CBitMemStream::serialCont(std::vector<bool> &cont)
{
	...
	if(isReading())
	{
		...
		{
			throw SIPBASE::EInvalidDataStream( "BMS: Trying to read a vec<bool> of %u bytes, past stream size", len/8 );
		}
...
#ifdef LOG_ALL_TRAFFIC
	if ( VerboseAllTraffic )
		sipdebug( "TRAFFIC/%p/%s: Container (header: 32 bits) at bitpos %d", this, isReading()?"I":"O", bitpos );
#endif
 ...
 */
void	CBitMemStream::displayLastBits( sint nbits, sint bitpos, SIPBASE::CLog *log )
{
	if ( bitpos == -1 )
		bitpos = getPosInBit();	
 ...
 */
void	displayBitStream( const CBitMemStream& msg, sint beginbitpos, sint endbitpos, SIPBASE::CLog *log )
{
	sint beginpos = beginbitpos/8;
	sint endpos = endbitpos/8;
	sipinfo( "BMS: beginpos %d endpos %d beginbitpos %d endbitpos %d", beginpos, endpos, beginbitpos, endbitpos );
...
{
//	sipassert( (_BufPos >= _Buffer.getPtr()) && (_BufPos <= _Buffer.getPtr() + _Buffer.size()) );
	sipassert( _Buffer.Pos <= _Buffer.getBuffer().size() );
...
//	s += SIPBASE::toString( "BitPos=%d Pos=%u FreeBits=%u Size=%u ", getPosInBit(), (uint32)(_BufPos-_Buffer.getPtr()), _FreeBits, _Buffer.size() );
	s += SIPBASE::toString( "BitPos=%d Pos=%u FreeBits=%u Size=%u ", getPosInBit(), _Buffer.Pos, _FreeBits, _Buffer.getBuffer().size() );
		...
		case TBMSSerialInfo::B:
			{
				bool b;
				serialBit( b );
				s = SIPBASE::toString( "%s", b?"TRUE":"FALSE" );
				break;
			}
		...
		case TBMSSerialInfo::U64: // no distinction with signed int64!
			{
				uint64 u;
				serial( u, serialItem.BitSize );
				s = SIPBASE::toString( "%" SIP_I64 "u", u );
				break;
			}
