...
/*static void PerformStatistics(const std::vector<double> &values, double &standardDeviation)
{
	...
	for(k = 0; k < values.size(); ++k)
	{
		variance += SIPBASE::sqr((values[k] - meanValue));
	}
...
void	CHTimer::estimateAfterStopTime()
{
	...
	{
		static SIPBASE::CHTimer		estimateSampleTimer("sampleTimer");
...
void	CHTimer::endBench()
{
	...
	{
		sipwarning("HTIMER: Stopping the bench inside a benched functions !");
	}
	_Benching = false;
...
{	
	...
	if (displayInline)
	{
		...
		format = "HTIMER: %-" + SIPBASE::toStringA(maxNodeLenght + 1) + "s %s";
	}
	...
	for(TTimerStatPtrVect::iterator statIt = statsPtr.begin(); statIt != statsPtr.end(); ++statIt)
	{
		...
		else
		{
			...
			char out[4096];
			SIPBASE::smprintf(out, 2048, format.c_str(), (*statIt)->Timer->_Name, statsInline.c_str());
...
{	
	...
	sipassert(_BenchStartedOnce); // should have done at least one bench	
	//
	...
	if (displayInline)
	{
		if (alignPaths)
		{
			...
			format = "HTIMER: %-" + SIPBASE::toStringA(maxSize) +"s %s";
		}
		else
		{
			format = "HTIMER: %s %s";
	...
	for(TNodeStatPtrVect::iterator it = nodeStatsPtrs.begin(); it != nodeStatsPtrs.end(); ++it)
	{
		...
		else
		{
			...
			char out[2048];
			SIPBASE::smprintf(out, 2048, format.c_str(), nodePath.c_str(), statsInline.c_str());
			log->displayRawNL(out);
		}
	}
	benchClock.stop();
...
void CHTimer::CStats::buildFromNodes(CNode **nodes, uint numNodes, double msPerTick)
{
	...
	for(k = 0; k < numNodes; ++k)
	{
		...
		{
			varianceSum += SIPBASE::sqr(nodes[k]->Measures[l] - MeanTime);
		}		
	}
...
void CHTimer::CStats::display(CLog *log, bool displayEx, bool wantStandardDeviation /* = false*/)
{
	...
	log->displayRawNL(("HTIMER: Num visits                = " + SIPBASE::toStringA(NumVisits)).c_str());	
	if (displayEx)
	{
...
void CHTimer::CStats::getStats(std::string &dest, bool statEx, double rootTotalTime, bool wantStandardDeviation /*= false*/)
{
	...
	{	
		if (!statEx)
		{	
			SIPBASE::smprintf(buf, 1024, " | %10.3f | %10.3f | %12s ", (float) TotalTime, (float) TotalTimeWithoutSons, toStringA(NumVisits).c_str());
		}
		else
		{
			SIPBASE::smprintf(buf, 1024, " | %10.3f | %10.3f | %12s | %5.1f/%5.1f | %9.3f | %9.3f | %9.3f | %9.3f",
					  (float) TotalTime, (float) TotalTimeWithoutSons, toStringA(NumVisits).c_str(), 
	...
	else
	{
		if (!statEx)
		{	
			SIPBASE::smprintf(buf, 1024, " | %10.3f | %10.3f | %12s | std deviation %9.3f", (float) TotalTime, (float) TotalTimeWithoutSons, toStringA(NumVisits).c_str(), (float) TimeStandardDeviation);
		}
		else
		{
			SIPBASE::smprintf(buf, 1024, " | %10.3f | %10.3f | %12s | %5.1f/%5.1f | %9.3f | %9.3f | %9.3f | %9.3f | std deviation %9.3f",
							  (float) TotalTime, (float) TotalTimeWithoutSons, toStringA(NumVisits).c_str(), 
							  float(100*TotalTimeWithoutSons/rootTotalTime), float(100*TotalTime/rootTotalTime), 
							  (float) SessionMaxTime,
							  (float) MinTime, (float) MaxTime, (float) MeanTime,
...
bool CHTimer::CStatSorter::operator()(const CHTimer::CStats *lhs, const CHTimer::CStats *rhs)
{
	switch(Criterion)
	{
		...
		default:
			sipassert(0); // not a valid criterion
		break;
	}
	return false;
}
...
void	CHTimer::doAfter(bool displayAfter)
{
	...
	*/
	...
	if (displayAfter)
	{		
		sipinfo("HTIMER: %s %.3fms loop number %d", _Name, numTicks * _MsPerTick, _CurrNode->NumVisits);
	}
	//
...
SIPBASE_CATEGORISED_COMMAND(sip, displayMeasures, "display hierarchical timer", "[TotalTime(=-2)|NoSort(=-3)|TotalTimeWithoutSons(=-4)|MeanTime(=-5)|NumVisits(=-6)|MaxTime(=-7)|MinTime(=-8)|MaxSession(=-9)] [depth]")
{
	if (args.size() < 1)
	{
		CHTimer::display(&log);
