...
/*!
 * \file	SipHttpClient.cpp
 * \brief	Implementations of HTTP client classes.
 * \since	2004.04.12
 * \version	$LastChangedRevision: 103 $
 *			$LastChangedDate: 2006-02-05 00:38:41 +0900
 *
 * \custom	Tinguowenming Team
 * \last	2011-12-21
 *
 * Used WinHTTP instead of WinINet
 * WinHTTP 5.1 is supported for computers running on Windows 2000 Server SP3, Windows XP SP1, and Windows Server 2003 family operating systems.
 *
 * 
 * Now, this module can be only for servers with no authentication
 */
...
#define MEDIA_HEADER_SIZE						1024

// An assertion macro for the CHttpToolW class
...
static LPCWSTR		g_WinInetMsgWin32W[] = {
	...
	, L"::WinHttpReadData failed."
	, L"::WinHttpOpen failed."
	, L"::WinHttpConnect failed."
	, L"::WinHttpOpenRequest failed."
	, L"::WinHttpAddRequestHeaders failed."
	, L"::WinHttpSendRequest failed."
	, L"::WinHttpSendRequest failed."
	, L"::WinHttpWriteData failed."
	, L"::WinHttpReceiveResponse failed."
	, L"::WinHttpSetOption failed."
} ;
...
LPCWSTR CHttpToolW::szDefUsrAgent = L"Sip HTTP Client Class" ;
...
LPCWSTR CHttpToolW::szDefBoundary = L"----Sip-FB3B405B7EAE495aB0C0295C54D4E096-" ;
LPCSTR CHttpToolW::szDefBoundaryA = "----Sip-FB3B405B7EAE495aB0C0295C54D4E096-" ;
LPCWSTR CHttpToolW::szDefUploadContType = L"multipart/form-data; boundary=" L"----Sip-FB3B405B7EAE495aB0C0295C54D4E096-" ;
	...
	throw (Exception &)
{
	...
	if ( NULL == (hInternet = ::WinHttpOpen (
		...
		ThrowException(HTTPCLIENT_ERR_WINHTTPOPEN_FAILED, ::GetLastError());
...
HINTERNET CHttpToolW::OpenConnection(HINTERNET hInternet, LPCWSTR szServerAddr, INTERNET_PORT nPort, LPCWSTR szUsrName, LPCWSTR szUsrPwd)
	throw (Exception &)
{
	...
	if (NULL == (hConnection = ::WinHttpConnect(
			hInternet
			, szServerAddr
			, nPort
			, 0)
			))
		ThrowException (HTTPCLIENT_ERR_WINHTTPCONNECT_FAILED, ::GetLastError()) ;
	...
	throw (Exception &)
{
	...
	if (NULL == (hRequest = ::WinHttpOpenRequest(
			hConnection
		...
		ThrowException (HTTPCLIENT_ERR_WINHTTPOPENREQUEST_FAILED, ::GetLastError()) ;
	...
	throw (Exception &)
{
	...
	try {
		cbHeader = ::wcslen (szName) ;
		cbHeader += szValue ? ::wcslen (szValue) : 0 ;
		cbHeader += (4 + 1) ;	// for L": ", L"\r\n", L'\0'
		cchHeader = cbHeader - 1 ;
	} catch (::SafeIntException & e) {
		ThrowException (e) ;
	}

	PWSTR		szHeader = (PWSTR) ::malloc (sizeof (WCHAR) * (cbHeader.Value ())) ;
	if (szHeader == NULL)
		ThrowException(HTTPCLIENT_ERR_OUT_OF_MEMORY);

	::wcscpy (szHeader, szName);
	::wcscat (szHeader, L": ");
	::wcscat (szHeader, szValue ? szValue : L"");
	::wcscat (szHeader, L"\r\n");
	...
	if ( !::WinHttpAddRequestHeaders (
		...
		, WINHTTP_ADDREQ_FLAG_ADD				// flags
		)
		)
	{
			SAFEFREE (szHeader) ;
			ThrowException (HTTPCLIENT_ERR_WINHTTPADDREQUESTHEADERS_FAILED, ::GetLastError()) ;
	}
	...
	throw (Exception &)
{
	...
	if ( !::WinHttpSendRequest (
			hRequest
			, WINHTTP_NO_ADDITIONAL_HEADERS	// Additional header
			, 0								// The length of the additional header
			, (void *) szPostedA			// A posted data
			, cchPosted.Value()				// The length of the posted data
			, cchPosted.Value()				// Content-Length
			, 0
		) )
	{
		SAFEFREE (szPostedA) ;
		ThrowException (HTTPCLIENT_ERR_WINHTTPSENDREQUEST_FAILED, ::GetLastError());
	}
	SAFEFREE (szPostedA) ;
}
	...
	throw (Exception &)
{
	...
	if (!::WinHttpSendRequest(
				...
				, 0								// The length of the additional header
				, 0								// A posted data
				, 0								// The length of the posted data
				, dwPostedSize					// Content-Length
				, 0
		))
		ThrowException(HTTPCLIENT_ERR_WINHTTPSENDREQUESTEX_FAILED, ::GetLastError());
}
	...
	throw (Exception &)
{
	...
	while (dwTotalWritten < cbyBuff)
	{
		if ( !::WinHttpWriteData (
			...
			ThrowException (HTTPCLIENT_ERR_WINHTTPWRITEDATA_FAILED, ::GetLastError());

		dwTotalWritten += dwWritten ;
	}
}

void CHttpToolW::EndRequest(HINTERNET hRequest, BOOL bIgnoreTimeout)
	throw (Exception &)
{
	...
	{
		DWORD dwWin32LastError = ::GetLastError();

//		if (!bIgnoreTimeout || (dwWin32LastError != ERROR_WINHTTP_TIMEOUT))
		if (!bIgnoreTimeout)
			ThrowException (HTTPCLIENT_ERR_WINHTTPRECEIVERESPONSE_FAILED, dwWin32LastError);
	}
}
...
// (Does not throw an exception)
HANDLE CHttpToolW::OpenFile (LPCWSTR szFilePath, BOOL bShareWrite)
	throw (Exception &)
{
	...
	throw (Exception &)
{
	...
	if (!::WinHttpQueryHeaders(
			hRequest
			, WINHTTP_QUERY_STATUS_TEXT 		// Get the status text
		...
		if (::GetLastError () != ERROR_INSUFFICIENT_BUFFER)
			ThrowException(HTTPCLIENT_ERR_WINHTTPQUERYHEADERS_FAILED, ::GetLastError());
...
// The returned string must be freed by using the ::free () function.
// pnIdx exactly corresponds to the lpdwIndex parameter in the ::WinHttpQueryHeaders function.
	...
	throw (Exception &)
{
	...
	if ( ::WinHttpQueryHeaders (
			hRequest
			, WINHTTP_QUERY_CUSTOM 				// Get a custom header
			, szName
...
void CHttpToolW::InternetSetOption (HINTERNET hInternet, DWORD dwOption, LPVOID lpBuffer, DWORD dwBufferLength)
	throw (Exception &)
{
	if (::WinHttpSetOption(hInternet, dwOption, lpBuffer, dwBufferLength))
		return ;

	ThrowException (HTTPCLIENT_ERR_WINHTTPSETOPTION_FAILED, ::GetLastError ()) ;
}
...
template <typename HttpTool>
DWORD CHttpClientMapT<HttpTool>::GetUserData (DWORD nIdx)
	throw ()
{
	return Get (nIdx).dwUserData ;
}
...
template <typename HttpTool>
DWORD CHttpClientMapT<HttpTool>::GetUserData (PCSZ szName, DWORD nIdx)
throw ()
{
	return Get (szName, nIdx).dwUserData ;
}
...
template <typename HttpTool>
void CHttpClientMapT<HttpTool>::AddPointerDirectly (PSZ szName, PSZ szValue, DWORD dwFlag, DWORD dwUserData)
	throw (Exception &)
{
	...
	try {
		...
		MapValue	newValue = { (PCSZ) szValue, dwFlag, dwUserData } ;
...
// The szValue could be NULL.
template <typename HttpTool>
void CHttpClientMapT<HttpTool>::Add(PCSZ szName, PCSZ szValue, DWORD dwFlag, DWORD dwUserData)
	throw (Exception &)
{
	...
	try {
		...
		AddPointerDirectly (szNewName, szNewValue, dwFlag, dwUserData) ;
...
// The szValue could be NULL.
template <typename HttpTool>
void CHttpClientMapT<HttpTool>::Set(PCSZ szName, PCSZ szValue, DWORD dwFlag, DWORD dwUserData, DWORD nIdx)
	throw (Exception &)
{
	...
	if ( pairIter.first == pairIter.second ) {
		// The nIdx must be 0
		_ASSERTE ( nIdx == 0 ) ;
		Add(szName, szValue, dwFlag, dwUserData) ;
		return ;
	}
	...
	MapValue		newValue = { NULL, dwFlag, dwUserData } ;
	if ( szValue ) {
	...
	throw (Exception &)
{
	...
	HTTPCLIENT_ASSERT (szBuff != NULL, "CHttpEncoderW::AnsiEncode: szBuff can not be NULL.") ;
	...
	throw (Exception &)
{
	...
	HTTPCLIENT_ASSERT (szBuff != NULL, "CHttpEncoderW::AnsiDecode: szBuff can not be NULL.") ;
...
PSTR CHttpEncoderW::Utf8Encode (PSTR szBuff, PCWSTR szStr, UINT /* CodePage */)
	throw (Exception &)
{
	HTTPCLIENT_ASSERT (szBuff != NULL, "CHttpEncoderW::Utf8Encode: szBuff can not be NULL.") ;
...
PWSTR CHttpEncoderW::Utf8Decode (PWSTR szBuff, PCSTR szEncoded, UINT /* CodePage */)
	throw (Exception &)
{
	HTTPCLIENT_ASSERT (szBuff != NULL, "CHttpEncoderW::Utf8Decode: szBuff can not be NULL.") ;
...
PSTR CHttpEncoderW::UrlEncodeA (PSTR szBuff, PCWSTR szStr, BOOL bUtf8Encoding, UINT CodePage)
	throw (Exception &)
{
	HTTPCLIENT_ASSERT (szBuff != NULL, "CHttpEncoderW::UrlEncodeA: szBuff can not be NULL.") ;
...
PWSTR CHttpEncoderW::UrlEncodeW (PWSTR szBuff, PCWSTR szStr, BOOL bUtf8Encoding, UINT CodePage)
	throw (Exception &)
{
	HTTPCLIENT_ASSERT (szBuff != NULL, "CHttpEncoderW::UrlEncodeW: szBuff can not be NULL.") ;
...
void CHttpEncoderW::_Utf8CharToAnsiChar (PSTR szAnsiChar, PCSTR szUtf8Char, UINT CodePage)
	throw (Exception &)
{
	HTTPCLIENT_ASSERT (szAnsiChar != NULL && szUtf8Char != NULL
...
PSTR CHttpEncoderW::UrlDecodeA (PSTR szBuff, PCWSTR szEncoded, BOOL bUtf8Encoding, UINT CodePage)
	throw (Exception &)
{
	HTTPCLIENT_ASSERT (szBuff != NULL, "CHttpEncoderW::UrlDecodeA: szBuff can not be NULL.") ;
...
PWSTR CHttpEncoderW::UrlDecodeW (PWSTR szBuff, PCWSTR szEncoded, BOOL bUtf8Encoding, UINT CodePage)
	throw (Exception &)
{
	HTTPCLIENT_ASSERT (szBuff != NULL, "CHttpEncoderW::UrlDecodeW: szBuff can not be NULL.") ;
 ...
 *
 * \param hInternet		[in] An internet handle returned by ::WinHttpOpen.
 * \param hConnection	[in] A connection handle returned by ::WinHttpConnect.
 * \param hRequest		[in] A request handle returned by ::WinHttpSendRequest or ::WinHttpReceiveResponse function.
 ...
 * The user also has to manage the lifetime of the internet handle returned by ::WinHttpOpen
 ...
 * user has to manage the lifetime of the internet handles returned by ::WinHttpOpen and ::WinHttpConnect
 * until the CHttpResponseT is destoried.
 *
 * \param hRequest		[in] A request handle returned by ::WinHttpSendRequest or ::WinHttpReceiveResponse function.
 *						     The hRequest parameter can not be NULL.
	...
	throw (Exception &)
{
	...
	if ( !::WinHttpQueryHeaders (
			m_hRequest
			, WINHTTP_QUERY_STATUS_CODE 		// Get the HTTP status code
		...
		HttpTool::ThrowException(HTTPCLIENT_ERR_WINHTTPQUERYHEADERS_FAILED, ::GetLastError());
	...
	throw (Exception &)
{
	...
	if ( !::WinHttpQueryHeaders (
			m_hRequest
			, WINHTTP_QUERY_CONTENT_LENGTH 		// Reads the 'content-length' header
				| WINHTTP_QUERY_FLAG_NUMBER 
			, WINHTTP_HEADER_NAME_BY_INDEX
	...
	throw (Exception &)
{
	...
	if (!::WinHttpReadData(
		...
		HttpTool::ThrowException(HTTPCLIENT_ERR_WINHTTPREADDATA_FAILED, ::GetLastError());
...
template <typename HttpTool>
void CHttpPostStatT<HttpTool>::CorrectCurrParamTotalByte(size_t cbSize)
throw (Exception &)
{
	HTTPCLIENT_ASSERT (m_bIsActive, "CHttpPostStatT::CurrParamTotalByte: The post context is not active.") ;

	if (!m_bCurrParamCorrect)
	{
		m_bCurrParamCorrect = TRUE;
		if (m_cbCurrParam != cbSize)
		{
			m_cbTotal -= m_cbCurrParam;
			m_cbTotal += cbSize;

			m_cbActualTotal -= m_cbCurrParam;
			m_cbActualTotal += cbSize;

			m_cbCurrParam = cbSize;
		}
	}
}
...
template <typename HttpTool>
BOOL CHttpPostStatT<HttpTool>::IsCurrParamTotalByteCorrect(void) const
throw (Exception &)
{
	HTTPCLIENT_ASSERT (m_bIsActive, "CHttpPostStatT::IsCurrParamTotalByteCorrect: The post context is not active.") ;
	return m_bCurrParamCorrect;
}
	...
	throw ()
{
	...
	if (!IsCurrParamTotalByteCorrect())
		return FALSE;
 ...
 */
template <typename HttpTool>
void CHttpPostStatT<HttpTool>::_StartNewEntry (PCSZ szCurrParam, size_t cbCurrParam, BOOL bIsCorrect
												, BOOL bIsFile, PCSZ szCurrFile)
	throw (Exception &)
{
	...
	m_bCurrParamCorrect = bIsCorrect;
	...
	throw ()
{
	...
	m_bCurrParamCorrect = objPostStat.m_bCurrParamCorrect;
	...
	throw ()
{
	...
	m_dwInternetAccessType = WINHTTP_ACCESS_TYPE_DEFAULT_PROXY;
 ...
 */
template <typename HttpTool, typename HttpEncoder>
void CHttpClientT<HttpTool, HttpEncoder>::AddParam (PCSZ szName, PCSZ szValue, DWORD dwParamAttr, DWORD dwUserData)
	throw (Exception &)
{
	// It is not allowed to call this method if the POST context is active.
	HTTPCLIENT_ASSERT (!_PostContextIsActive ()
		, "CHttpClientT::AddParam: It is not allowed to call this method if the POST context is active.") ;
	m_mapParam.Add (szName, szValue, dwParamAttr, dwUserData) ;
}
 ...
 */
template <typename HttpTool, typename HttpEncoder>
void CHttpClientT<HttpTool, HttpEncoder>::SetParam (PCSZ szName, PCSZ szValue, DWORD dwParamAttr, DWORD dwUserData, DWORD nIdx)
	throw (Exception &)
{
	// It is not allowed to call this method if the POST context is active.
	HTTPCLIENT_ASSERT (!_PostContextIsActive ()
		, "CHttpClientT::SetParam: It is not allowed to call this method if the POST context is active.") ;
	m_mapParam.Set (szName, szValue, dwParamAttr, dwUserData, nIdx) ;
}
...
/*!
 * This method sets parameters for the ::WinHttpOpen function which is called internally before sending a request.
 * All parameters passed to this method is forwarded to the ::WinHttpOpen function.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 ...
 * \endcode
 *
 * \param szUserAgent		[in] A User Agent which corresponds to the lpszAgent parameter of the ::WinHttpOpen function.
 * \param dwAccessType		[in] A type of access which corresponds to the dwAccessType parameter of the ::WinHttpOpen function.
 * \param szProxyName		[in] A name of a proxy server which corresponds to the lpszProxyName parameter of the ::WinHttpOpen function.
 * \param szProxyBypass		[in] A proxy bypass list which corresponds to the lpszProxyBypass parameter of the ::WinHttpOpen function.
 * \param dwFlags			[in] Options which corresponds to the dwFlags parameter of the ::WinHttpOpen function.
 * \throw					Throws a httpclientexception if an error occurs.
 *
 * \sa		::WinHttpOpen
 */
...
/*!
 * This method sets the lpszAgent parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * The parameter passed to this method is forwarded to the ::WinHttpOpen function.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \param szUserAgent		[in] A User Agent which corresponds to the lpszAgent parameter of the ::WinHttpOpen function.
...
/*!
 * This method returns the lpszAgent parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \return					A User Agent which corresponds to the lpszAgent parameter of the ::WinHttpOpen function.
 *
 * \sa		::WinHttpOpen, SetInternet
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method sets the dwAccessType parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * The parameter passed to this method is forwarded to the ::WinHttpOpen function.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \param dwAccessType		[in] A type of access which corresponds to the dwAccessType parameter of the ::WinHttpOpen function.
...
/*!
 * This method returns the dwAccessType parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \return					A type of access which corresponds to the dwAccessType parameter of the ::WinHttpOpen function.
 *
 * \sa		::WinHttpOpen, SetInternet
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method sets the lpszProxyName parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * The parameter passed to this method is forwarded to the ::WinHttpOpen function.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \param szProxyName		[in] A proxy server name which corresponds to the lpszProxyName parameter of the ::WinHttpOpen function.
 * \throw					Throws a httpclientexception if an error occurs.
 *
 * \sa		::WinHttpOpen, SetInternet
 */
...
/*!
 * This method returns the lpszProxyName parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \return					A proxy server name which corresponds to the lpszProxyName parameter of the ::WinHttpOpen function.
 *
 * \sa		::WinHttpOpen, SetInternet
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method sets the lpszProxyBypass parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * The parameter passed to this method is forwarded to the ::WinHttpOpen function.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \param szProxyBypass		[in] A proxy bypass list which corresponds to the lpszProxyBypass parameter of the ::WinHttpOpen function.
...
/*!
 * This method returns the lpszProxyBypass parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \return					A proxy bypass list which corresponds to the lpszProxyBypass parameter of the ::WinHttpOpen function.
 *
 * \sa		::WinHttpOpen, SetInternet
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method sets the dwFlags parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * The parameter passed to this method is forwarded to the ::WinHttpOpen function.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \param dwFlags			[in] Options which corresponds to the dwFlags parameter of the ::WinHttpOpen function.
...
/*!
 * This method returns the dwFlags parameter for the ::WinHttpOpen function which is called internally before sending a request.
 * For more infomation about the ::WinHttpOpen function, see the MSDN documentation.
 *
 * \return					Options which corresponds to the dwFlags parameter of the ::WinHttpOpen function.
 *
 * \sa		::WinHttpOpen, SetInternet
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method returns a new HINTERNET handle which is returned by ::WinHttpOpen
...
/*!
 * This method returns a new HINTERNET handle which is returned by ::WinHttpConnect.
 ...
 * The caller is responsible for closing the returned handle when it has finished with it.
 *
 * \param hInternet		[in] A valid handle returned by ::WinHttpOpen.
 * \param szUrl			[in] An URL which is used to return a new handle.
 * \param szUsrName		[in] An user name which corresponds to the lpszUsername parameter of the ::WinHttpConnect function.
 * \param szUsrPwd		[in] An user password which corresponds to the lpszPassword parameter of the ::WinHttpConnect function.
 * \return				A new HINTERNET handle.
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa	::WinHttpConnect
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa	ApplyProxyAccount, ::WinHttpSetOption, <a href="http://support.microsoft.com/?kbid=195650" target="_blank">How To Handle Proxy Authorization with WinInet</a>
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * \endcode
 *
 * \param hConnection	[in] A HINTERNET handle returned by ::WinHttpConnect function.
 *							 NULL is not allowed.
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa	SetProxyAccount, ::WinHttpSetOption, <a href="http://support.microsoft.com/?kbid=195650" target="_blank">How To Handle Proxy Authorization with WinInet</a>
 */
template <typename HttpTool, typename HttpEncoder>
...
/*!
 * This method returns a new HINTERNET handle which is returned by ::WinHttpOpenRequest
 * using the settings saved in the CHttpClient.
 ...
 * The caller is responsible for closing the returned handle when it has finished with it.
 *
 * \param hConnection	[in] A valid handle returned by ::WinHttpConnect.
 * \param szVerb		[in] A HTTP verb which corresponds to the lpszVerb parameter of the ::WinHttpOpenRequest function.
 * \param szUrl			[in] An URL which is used to return a new handle.
 * \param dwFlags		[in] A flags which corresponds to the dwFlags parameter of the ::WinHttpOpenRequest function.
 * \param szReferer		[in] A referer which corresponds to the lpszReferer parameter of the ::WinHttpOpenRequest function.
 * \return				A new HINTERNET handle.
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa	::WinHttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
	...
	throw (Exception &)
{
	...
	try {
		...
		if ( dwFlags & WINHTTP_FLAG_REFRESH )
...
/*!
 * This method adds the HTTP headers to the handle which is returned by ::WinHttpOpenRequest
 * using the settings saved in the CHttpClient.
 ...
 *
 * \param hRequest	[in] A valid handle which is returned by ::WinHttpOpenRequest.
 * \throw			Throws a httpclientexception if an error occurs.
 *
 * \sa		::WinHttpAddRequestHeaders
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * If the caller passes the hInternet parameter which is returned by ::WinHttpOpen,
 ...
 * If the caller passes the hConnection parameter which is returned by ::WinHttpConnect,
 ...
 *
 * \param hInternet		[in] A valid handle returned by ::WinHttpOpen. NULL is allowed.
 * \param hConnection	[in] A valid handle returned by ::WinHttpConnect. NULL is allowed.
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		CHttpResponseT, OpenInternet, OpenConnection, ::WinHttpOpen, ::WinHttpConnect
 */
template <typename HttpTool, typename HttpEncoder>
	...
	throw (Exception &)
{
	return RequestGetEx (hInternet, hConnection, szUrl
		, bUseCache ? 0 : WINHTTP_FLAG_REFRESH) ;
}
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		CHttpResponseT, OpenInternet, ::WinHttpOpen
 */
template <typename HttpTool, typename HttpEncoder>
typename CHttpClientT<HttpTool, HttpEncoder>::CHttpResponse *
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		CHttpResponseT, OpenInternet, OpenConnection, ::WinHttpOpen, ::WinHttpConnect, ::WinHttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
typename CHttpClientT<HttpTool, HttpEncoder>::CHttpResponse * 
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		CHttpResponseT, OpenInternet, ::WinHttpOpen, ::WinHttpConnect, ::WinHttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
typename CHttpClientT<HttpTool, HttpEncoder>::CHttpResponse * 
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		CHttpResponseT, ::WinHttpConnect, ::WinHttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
typename CHttpClientT<HttpTool, HttpEncoder>::CHttpResponse * 
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		OpenInternet, OpenConnection, ::WinHttpOpen, ::WinHttpConnect
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		Query, Proceed, OpenInternet, OpenConnection, ::WinHttpOpen, ::WinHttpConnect, ::WinHttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		Query, Proceed, OpenInternet, ::WinHttpOpen, ::WinHttpConnect, ::WinHttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		Query, Proceed, ::WinHttpConnect, ::WinHttpOpenRequest
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		Query, Proceed, OpenInternet, OpenConnection, ::WinHttpOpen, ::WinHttpConnect
 */
template <typename HttpTool, typename HttpEncoder>
 ...
 * \throw				Throws a httpclientexception if an error occurs.
 *
 * \sa		Query, Proceed, OpenInternet, ::WinHttpOpen
 */
template <typename HttpTool, typename HttpEncoder>
...
template <typename HttpTool, typename HttpEncoder>
typename CHttpClientT<HttpTool, HttpEncoder>::CHttpResponse * 
CHttpClientT<HttpTool, HttpEncoder>::Proceed(DWORD cbDesired, BOOL bMediaOk)
	throw (Exception &)
{
	if ( _PostContextIsPost () )
		return _ProceedPostContext (cbDesired) ;

	return _ProceedUploadContext (cbDesired, bMediaOk) ;
}
		...
		if ( m_objPostStat.CurrParamIsComplete () ) {
			...
			if ( (iter->second).szValue ) {
				...
				if ( (iter->second).dwFlag & ParamEncodedValue ) {
					...
					cbValue = szValueA ? ::strlen(szValueA) : 0 ;
				} else {
					cbValue = _UrlEncodeLenA ((iter->second).szValue) ;
...
LPCSTR CHttpClientT<HttpTool, HttpEncoder>::_GetUploadBoundaryA (void)
	throw ()
{
	return m_szBoundaryA ? m_szBoundaryA : HttpTool::szDefBoundaryA;
}
	...
	throw (Exception &)
{
	...
	try
	{
		// Calculates the number of bytes to upload
		::SafeInt<size_t>	nTotalByte = 0 ;
		::SafeInt<size_t>	nValuesTotalByte = 0 ;
		...
		if (!m_mapParam.Empty ())
		{
			try
			{
				...
				/*
					...
					<3> The number of bytes to upload If the parameter is a media file
						= <2> + strlen("--") + strlen(Boundary) + strlen("++\r\n") 
							+ MEDIA_HEADER_SIZE + strlen("\r\n")
					...
					The total upload size
						= ( <1> * The number of normal parameters)
							+ ( <2> or <3> * The number of file parameters)
							+ The last boundary
				*/
				...
				for (iter = m_mapParam.Begin (); iter != m_mapParam.End (); ++iter)
				{
					...
					else
					{
						...
						//		+ strlen ("Content-Type: \r\n\r\n") + strlen (The Mime Type of the file)
						//		+ The length of the file + strlen ("\r\n")
						// If the parameter is a media file
						//	= <2> + strlen("--") + strlen(Boundary) + strlen("++\r\n") 
						//		+ MEDIA_HEADER_SIZE + strlen("\r\n")
						...
						if ((iter->second).szValue)
						{
							if (((iter->second).dwFlag & ParamMedia) == ParamMedia)
								// Get the media's predicted size
								cbValue = (iter->second).dwUserData;
							else if (ahFileHandles[nFileIdx] == INVALID_HANDLE_VALUE)
							{
								// Open the file
						...
						nTotalByte += ::strlen (aszMimeTypes[nFileIdx]) ;
						nTotalByte += cbValue ;
						nValuesTotalByte += cbValue ;
						nTotalByte += 2 ;
		...
		if ( !m_mapParam.Empty () ) {
			...
			// It always does not throw an overflow exception.
			// So it's safe. (doesn't need to restore the internal states)
			m_objInitialStat._StartNewEntry (szFirstParamName, cbFirstParamValue, bFirstParamCorrect
...
template <typename HttpTool, typename HttpEncoder>
typename CHttpClientT<HttpTool, HttpEncoder>::CHttpResponse * 
CHttpClientT<HttpTool, HttpEncoder>::_ProceedUploadContext(DWORD nDesired, BOOL bMediaOk)
	throw (Exception &)
{
	// If the Post context is not started
	if (!m_objPostStat.IsActive ())
	...
	try 
	{
		...
		{
			HttpTool::EndRequest(m_hRequest, bMediaOk); // if media uploading succeeded, occurs time-out.(need to check http server's code)
			return _ReleasePostResponse();
		}
		...
		BOOL IsCurrParamComplete = FALSE;
		DWORD nNextIdx		= m_objPostStat.PostedCount();
		...
		if (nNextIdx > 0)
		{
			DWORD nCurrIdx = nNextIdx - 1;
			for (size_t i = 0; i < nCurrIdx; i++, ++iter);

			if ((((iter->second).dwFlag & ParamMedia) != ParamMedia) || m_objPostStat.IsCurrParamTotalByteCorrect())
				IsCurrParamComplete = m_objPostStat.CurrParamIsComplete();
			else if (bMediaOk)
			{
				DWORD nCurrFileIdx = nNextFileIdx - 1;
				if (m_ahPostedFiles[nCurrFileIdx] == INVALID_HANDLE_VALUE)
				{
					m_ahPostedFiles[nCurrFileIdx] = HttpTool::OpenFile((iter->second).szValue, TRUE);
					if (m_ahPostedFiles[nCurrFileIdx] == INVALID_HANDLE_VALUE)
						return NULL;
				}
				m_objPostStat.CorrectCurrParamTotalByte(HttpTool::GetFileSize(m_ahPostedFiles[nCurrFileIdx], (iter->second).szValue));
				
				IsCurrParamComplete = m_objPostStat.CurrParamIsComplete();
				if (IsCurrParamComplete)
				{
					_WritePost("\r\n--");
					_WritePost(szBoundary);
					_WritePost("++\r\n");

					::SetFilePointer(m_ahPostedFiles[nCurrFileIdx], 0, 0, FILE_BEGIN);
					DWORD cbRead;
					if (!::ReadFile(m_ahPostedFiles[nCurrFileIdx], m_pbyCntxBuff, MEDIA_HEADER_SIZE, &cbRead, NULL))
						HttpTool::ThrowException(HTTPCLIENT_ERR_READFILE_FAILED, ::GetLastError());

					if (MEDIA_HEADER_SIZE != cbRead)
						HttpTool::ThrowException(HTTPCLIENT_ERR_READ_UNEXPECTED_SIZE) ;

					_WritePost(m_pbyCntxBuff, cbRead);
		...
		if (IsCurrParamComplete)
		{
			...
			if ((iter->second).dwFlag & ParamFile)
			{
				...
				if (((iter->second).dwFlag & ParamMedia) == ParamMedia)
				{
					...
					{
						m_ahPostedFiles[nNextFileIdx] = HttpTool::OpenFile(szEntryFile, TRUE);
						if (m_ahPostedFiles[nNextFileIdx] == INVALID_HANDLE_VALUE)
							return NULL;
					}

					bEntryValueCorrect = FALSE;
					nEntryValueTotalByte = (iter->second).dwUserData;
				}
				else if (m_ahPostedFiles[nNextFileIdx] != INVALID_HANDLE_VALUE)
			...
			else
			{
				...
				if ((iter->second).szValue && (iter->second).szValue[0] != '\0')
				{
					...
					nEntryValueTotalByte = ::strlen (szPostedValue) ;
			...
			// Starts a new parameter
			m_objPostStat._StartNewEntry(iter->first, nEntryValueTotalByte.Value(), bEntryValueCorrect,
		...
		if (m_objPostStat.CurrParamIsFile())
		{
			...
			DWORD			nFileIdx = nNextIdx - 1;

			while (cbWritten < cbToWrite)
			{
				cbBuff = cbToWrite - cbWritten;
				if (cbBuff > HTTPCLIENT_POSTCNTX_BUFF_SIZE)
					cbBuff = HTTPCLIENT_POSTCNTX_BUFF_SIZE;
				...
				if ((cbBuff != cbRead) && m_objPostStat.IsCurrParamTotalByteCorrect())
					HttpTool::ThrowException (HTTPCLIENT_ERR_READ_UNEXPECTED_SIZE) ;
				
				if (cbRead == 0)
					break;
				
				_WritePost(m_pbyCntxBuff, cbRead, TRUE);
				cbWritten += cbRead;
			}
			...
			if (m_objPostStat.CurrParamRemainByte() == 0)
			{
				if (((iter->second).dwFlag & ParamMedia) == ParamMedia)
				{
					...
					::SetFilePointer(m_ahPostedFiles[nFileIdx], 0, 0, FILE_BEGIN);
					if (!::ReadFile(m_ahPostedFiles[nFileIdx], m_pbyCntxBuff, MEDIA_HEADER_SIZE, &cbRead, NULL))
						HttpTool::ThrowException(HTTPCLIENT_ERR_READFILE_FAILED, ::GetLastError());
