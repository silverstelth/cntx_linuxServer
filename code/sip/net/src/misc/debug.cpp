...
// it to 0.
#ifndef SIP_DEFAULT_DISPLAYER
#define SIP_DEFAULT_DISPLAYER 1
#endif // SIP_DEFAULT_DISPLAYER

// Put 0 if you don't want to display in file "sip.log"
// Alternatively, you can use --without-logging when using configure to set
// it to 0.
#ifndef SIP_LOG_IN_FILE
#define SIP_LOG_IN_FILE 1
#endif // SIP_LOG_IN_FILE

#define DEFAULT_DISPLAYER SIP_DEFAULT_DISPLAYER

#define LOG_IN_FILE SIP_LOG_IN_FILE
...
CLog* CImposterLog::operator -> ()
{
	...
	{
		return (SIPBASE::ISipContext::getInstance().*_Accessor)();
	}
	return NULL;
}
...
CImposterLog	ErrorLog (&ISipContext::getErrorLog);
CImposterLog	WarningLog (&ISipContext::getWarningLog);
CImposterLog	InfoLog (&ISipContext::getInfoLog);
CImposterLog	DebugLog (&ISipContext::getDebugLog);
CImposterLog	AssertLog (&ISipContext::getAssertLog);
...
{
	ISipContext::getInstance().setNoAssert(!assert);
}

void sipFatalErrorA (const char *format, ...)
{
	char *str;
	SIPBASE_CONVERT_VARGS (str, format, 256/*SIPBASE::MaxCStringSize*/);

	ISipContext::getInstance().setDebugNeedAssert( SIPBASE::DefaultMsgBoxDisplayer==0 );

	SIPBASE::ErrorLog->displayNL (str);

	if (ISipContext::getInstance().getDebugNeedAssert())
	{
			SIPBASE_BREAKPOINT;
			throw EFatalError(); // added by Sim
	}
...
{
	ucchar *str;
	SIPBASE_CONVERT_VARGS_WIDE (str, format, 256/*SIPBASE::MaxCStringSize*/);
...
// the default behavior is to display all in standard output and to a file named "sip.log";
...
{
	ISipContext::getInstance().setDebugNeedAssert( false );
	SIPBASE::createDebug ();
	if (SIPBASE::DefaultMsgBoxDisplayer)
		SIPBASE::DefaultMsgBoxDisplayer->IgnoreNextTime = ignoreNextTime;
	else if(!ISipContext::getInstance().getNoAssert())
		ISipContext::getInstance().setDebugNeedAssert(true);
	SIPBASE::AssertLog->setPosition (line, file, funcName);
	if (exp)		SIPBASE::AssertLog->displayNL ("\"%s\" ", exp);
	else		SIPBASE::AssertLog->displayNL ("STOP");
}

bool _assertex_stop_1(bool &ignoreNextTime)
{
	if (SIPBASE::DefaultMsgBoxDisplayer)
		ignoreNextTime = SIPBASE::DefaultMsgBoxDisplayer->IgnoreNextTime;
	return ISipContext::getInstance().getDebugNeedAssert();
}
...
typedef struct _SIP_MINIDUMP_EXCEPTION_INFORMATION {  DWORD ThreadId;  PEXCEPTION_POINTERS ExceptionPointers;  BOOL ClientPointers;
} SIP_MINIDUMP_EXCEPTION_INFORMATION, *PSIP_MINIDUMP_EXCEPTION_INFORMATION;
typedef enum _SIP_MINIDUMP_TYPE
{
...
{
	HANDLE file = CreateFileA (SIP_CRASH_DUMP_FILE, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (file)
	{
		...
		if (hm)
		{
			BOOL (WINAPI* MiniDumpWriteDump)(
			  ...
			  DWORD ProcessId,
			  HANDLE hFile,
			  SIP_MINIDUMP_TYPE DumpType,
			  PSIP_MINIDUMP_EXCEPTION_INFORMATION ExceptionParam,
			  PSIP_MINIDUMP_EXCEPTION_INFORMATION UserStreamParam,
			  PSIP_MINIDUMP_EXCEPTION_INFORMATION CallbackParam
			) = NULL;
			*(FARPROC*)&MiniDumpWriteDump = GetProcAddress(hm, "MiniDumpWriteDump");
			if (MiniDumpWriteDump)
			{
				// OutputDebugStringA("writing minidump\r\n");
				SIP_MINIDUMP_EXCEPTION_INFORMATION eInfo;
			...
			{
				sipwarning ("Can't get proc MiniDumpWriteDump in dbghelp.dll");
			}
		}
		else
		{
			sipwarning ("Can't load dbghelp.dll");
		}
		CloseHandle (file);
	}
	else
		sipwarning ("Can't create mini dump file");
}
...
public:	
	...
	EDebug(EXCEPTION_POINTERS * pexp) : m_pexp(pexp) { sipassert(pexp != 0); createWhat(); }
	...
	void createWhat ()
	{
		...
		else
		{
			...
			if(!shortExc.empty() || !longExc.empty())
			{
				...
				progname = CFileA::getFilename(name);
				progname += " ";
			}
			...
			if(!shortExc.empty() || !longExc.empty())
			{
				...
				bool i = false;
				report (progname+shortExc, "", subject, _Reason, true, 1, true, 1, !isCrashAlreadyReported(), i, SIP_CRASH_DUMP_FILE);
...
/*		// ace hack
		...
		sipverify (SymInitialize(getProcessHandle(), NULL, FALSE) == TRUE);
...
//
		...
		{
			str = line.FileName;
			str += "(" + toStringA ((uint32)line.LineNumber) + ")";
...
//		sipinfo ("not parsed '%s'", parse.c_str());
...
//		sipinfo ("after parsing '%s'", str.c_str());
...
#pragma message ( "Smart crash enabled" )
	...
	// Ace: 'if' not activated because we can't debug if enabled: keeping only 0xACEACE for sipstop...
...
void getCallStackAndLogA (string &result, sint skipNFirst)
{
#ifdef SIP_OS_WINDOWS
	try
	{
...
void getCallStackAndLogW (std::basic_string<ucchar> &result, sint skipNFirst)
{
#ifdef SIP_OS_WINDOWS
	try
	{
	...
	{
		result += getUcsFromMbs(e.what());
	}
#else

	// there s no stack on GNU/Linux, only get the log without filters

	result += L"No callstack available\n";
	result += L"-------------------------------\n";
	result += L"\n";
	if (DefaultMemDisplayer)
	{
		result += L"Log with no filter:\n";
		result += L"-------------------------------\n";
		DefaultMemDisplayer->write (result);
	}
	else
	...
	if (CrashCallback)
	{
		result += L"User Crash Callback:\n";
		result += L"-------------------------------\n";
		static bool looping = false;
		if (looping)
		{
			result += L"******* WARNING: crashed in the user crash callback *******\n";
			looping = false;
		}
		else
		{
			looping = true;
			result += getUcsFromMbs(CrashCallback().c_str());
			looping = false;
		}
		result += L"-------------------------------\n";
	}	
...
{
	string p = CPath::standardizePath(dir);
	if(fd)
		fd->setParam(p + "sip.log");
}

void changeLogDirectoryW(const std::basic_string<ucchar> &dir)
{
	basic_string<ucchar> p = CPath::standardizePath (dir);
	if(fd)
		fd->setParam(p + L"sip.log");
}

#ifdef SIP_USE_UNICODE
void createDebug (const ucchar *logPath, bool logInFile)
#else
void createDebug (const char *logPath, bool logInFile)
#endif
{
	SIP_ALLOC_CONTEXT (_Debug)
	
	if (!ISipContext::getInstance().getAlreadyCreateSharedAmongThreads())
	{
...
//		if (!IsDebuggerPresent ())
		...
		ISipContext::getInstance().setErrorLog(new CLog (CLog::LOG_ERROR));
		ISipContext::getInstance().setWarningLog(new CLog (CLog::LOG_WARNING));
		ISipContext::getInstance().setInfoLog(new CLog (CLog::LOG_INFO));
		ISipContext::getInstance().setDebugLog(new CLog (CLog::LOG_DEBUG));
		ISipContext::getInstance().setAssertLog(new CLog (CLog::LOG_ASSERT));

		sd = new CStdDisplayer ("DEFAULT_SD");
		ISipContext::getInstance().setDefaultStdDisplayer(sd);

#ifdef SIP_OS_WINDOWS
		if (TrapCrashInDebugger || !IsDebuggerPresent ())
		{
			DefaultMsgBoxDisplayer = new CMsgBoxDisplayer ("DEFAULT_MBD");
			ISipContext::getInstance().setDefaultMsgBoxDisplayer(DefaultMsgBoxDisplayer);
		}
#endif
		...
		if (logInFile)
		{
				...
				{
					// we want the sip.log to be in the current directory
					// ucchar	tmpPath[1024];
					// fn += _wgetcwd(tmpPath, 1024);
					// fn += L"/";
				}
				fn += L"sip.log";
			#else
...
#endif // FINAL_VERSION
			ISipContext::getInstance().setDefaultFileDisplayer(fd);
		}
#endif // LOG_IN_FILE
		DefaultMemDisplayer = new CMemDisplayer ("DEFAULT_MD");
		ISipContext::getInstance().setDefaultMemDisplayer(DefaultMemDisplayer);

		initDebug2(logInFile);

		ISipContext::getInstance().setAlreadyCreateSharedAmongThreads(true);
...
SIPBASE_SAFE_SINGLETON_IMPL(CInstanceCounterManager);
...
SIPBASE_CATEGORISED_COMMAND(sip, displayInstanceCounter, "display the instance counters", "[<filter>]")
{
	string className;
	if (args.size() == 1)
		className = args[0];
...
SIPBASE_CATEGORISED_COMMAND(sip, resetInstanceCounterDelta, "reset the delta value for instance counter", "")
{
...
SIPBASE_CATEGORISED_COMMAND(sip, displayMemlog, "displays the last N line of the log in memory", "[<NbLines>]")
{
	uint nbLines;
...
#ifdef SIP_USE_UNICODE
	deque<basic_string<ucchar> >::const_iterator it;
	const deque<basic_string<ucchar> > &str = DefaultMemDisplayer->lockStrings ();
#else
...
SIPBASE_CATEGORISED_COMMAND(sip, resetFilters, "disable all filters on Sip loggers", "[debug|info|warning|error|assert]")
{
	if(args.size() == 0)
	{
...
SIPBASE_CATEGORISED_COMMAND(sip, addPositiveFilterDebug, "add a positive filter on DebugLog", "<filterstr>")
{
...
SIPBASE_CATEGORISED_COMMAND(sip, addNegativeFilterDebug, "add a negative filter on DebugLog", "<filterstr>")
{
	if(args.size() != 1) return false;
...
SIPBASE_CATEGORISED_COMMAND(sip, removeFilterDebug, "remove a filter on DebugLog", "[<filterstr>]")
{
	if(args.size() == 0)
		DebugLog->removeFilter((char *)NULL);
	else if(args.size() == 1)
...
SIPBASE_CATEGORISED_COMMAND(sip, displayFilterDebug, "display filter on DebugLog", "")
{
...
SIPBASE_CATEGORISED_COMMAND(sip, addPositiveFilterInfo, "add a positive filter on InfoLog", "<filterstr>")
{
	if(args.size() != 1) return false;
...
SIPBASE_CATEGORISED_COMMAND(sip, addNegativeFilterInfo, "add a negative filter on InfoLog", "<filterstr>")
{
	if(args.size() != 1) return false;
...
SIPBASE_CATEGORISED_COMMAND(sip, removeFilterInfo, "remove a filter on InfoLog", "[<filterstr>]")
{
	if(args.size() == 0)
		InfoLog->removeFilter((char *)NULL);
	else if(args.size() == 1)
...
SIPBASE_CATEGORISED_COMMAND(sip, displayFilterInfo, "display filter on InfoLog", "[d|i|w|e]")
{
	if(args.size() > 1) return false;
	if ( args.size() == 1 )
	{
...
SIPBASE_CATEGORISED_COMMAND(sip, addPositiveFilterWarning, "add a positive filter on WarningLog", "<filterstr>")
{
	if(args.size() != 1) return false;
...
SIPBASE_CATEGORISED_COMMAND(sip, addNegativeFilterWarning, "add a negative filter on WarningLog", "<filterstr>")
{
	if(args.size() != 1) return false;
...
SIPBASE_CATEGORISED_COMMAND(sip, removeFilterWarning, "remove a filter on WarningLog", "[<filterstr>]")
{
	if(args.size() == 0)
		WarningLog->removeFilter((char *)NULL);
	else if(args.size() == 1)
...
SIPBASE_CATEGORISED_COMMAND(sip, displayFilterWarning, "display filter on WarningLog", "")
{
	if(args.size() != 0) return false;
...
SIPBASE_CATEGORISED_COMMAND(sip, assert, "generate a failed sipassert()", "")
{
	if(args.size() != 0) return false;
	sipassertex (false, ("Assert generated by the assert command"));
	return true;
}

SIPBASE_CATEGORISED_COMMAND(sip, stop, "generate a sipstop()", "")
{
	if(args.size() != 0) return false;
	sipstopex (("Stop generated by the stop command"));
	return true;
}

SIPBASE_CATEGORISED_COMMAND(sip, abort, "generate a abort()", "")
{
	if(args.size() != 0) return false;
	abort();
	return true;
...
SIPBASE_CATEGORISED_COMMAND(sip, divbyzero, "generate a divide by zero", "")
{
	if(args.size() != 0) return false;
	float a=10,b=0;
	a /= b;
...
SIPBASE_CATEGORISED_COMMAND(sip, writeaccess, "write a uint8 value in an invalid address", "[<adr> [<value>]]")
{
	uint8 val = 123;
	uint8 *adr = (uint8*)0;
	if(args.size() >= 1)
...
#else
	  adr = (uint8*)(intptr_t)atoi(args[0].c_str());
#endif
...
SIPBASE_CATEGORISED_COMMAND(sip, readaccess, "read a uint8 value in an invalid address", "[<adr>]")
{
	uint8 val;
	uint8 *adr = (uint8*)0;
	if(args.size() == 1)
