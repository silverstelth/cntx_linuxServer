...
SIPBASE_SAFE_SINGLETON_IMPL(CCommandRegistry);
...
ICommand::ICommand(const char *categoryName, const char *commandName, const char *commandHelp, const char *commandArgs)
{
	...
	if (comm != LocalCommands->end ())
	{
		...
		sipstopex (("There are 2 commands that have the same name in the project (command name '%s'), skip the second definition", commandName));
	}
	else
	{
		// insert the new command in the map
		//sipinfo ("add command '%s'", commandName);
		CategoryName = categoryName;
		HelpString = commandHelp;
		CommandArgs = commandArgs;
		_CommandName = commandName;
		Type = Command;
		(*LocalCommands)[_CommandName] = this;
	}

	if (ISipContext::isContextInitialised())
	{
...
ICommand::~ICommand()
{
	...
	for (TCommand::iterator comm = LocalCommands->begin(); comm != LocalCommands->end(); comm++)
	{
		if ((*comm).second == this)
		{
			...
			// Yoyo: if no sipinfo()/sipwarning() (thus no createDebug(), thus no new CApplicationContext)
			// done in the .dll, it is possible that the sip context is never initialized
			if (ISipContext::isContextInitialised())
			{
...
void CCommandRegistry::registerCommand(ICommand *command)
{
	...
	{
//		sipwarning("There are 2 commands that have the same name in the project (command name '%s'), skip the second definition", command->getName().c_str());
		return;
	}
...
void CCommandRegistry::unregisterCommand(ICommand *command)
{
	...
	sipwarning("CCommandRegistry::unregisterCommand : the command '%s' is not registered", command->getName().c_str());
}
...
void CCommandRegistry::registerNamedCommandHandler(ICommandsHandler *handler, const std::string &className)
{
	...
	{
		sipwarning("CCommandRegistry : a commands handler with the name '%s' already exist, ignoring new candidat", name.c_str());
		return;
	}
	...
	{
		sipinfo("CCommandRegistry : adding commands handler for class '%s'", className.c_str());
	}
	...
	sipassert(chci._Commands.empty() || chci._Commands == commands);
...
void CCommandRegistry::unregisterNamedCommandHandler(ICommandsHandler *handler, const std::string &className)
{
	...
	if (it != _CommandsHandlersClass.end())
	{
		...
		{
			sipinfo("CCommandRegistry : removing commands handler for class '% s'", className.c_str());
...
bool CCommandRegistry::execute (const std::string &commandWithArgs, CLog &log, bool quiet, bool human)
{
	if (!quiet)
	{
		...
		#ifdef SIP_OS_WINDOWS ///KSR_ADD 2010_2_1
			log.displayNL (L"Executing command : '%s'", temp.c_str());
		#else
			log.displayNL (L"Executing command : '%S'", temp.c_str());
		#endif
	}
 ...
 */
void ICommand::expand (std::string &commandName, SIPBASE::CLog &log)
{
	// forward to command registry
	CCommandRegistry::getInstance().expand(commandName, log);
}

void CCommandRegistry::expand (std::string &commandName, SIPBASE::CLog &log)
{
...
SIPBASE_CATEGORISED_COMMAND(sip, help,"display help on a specific variable/commands or on all variables and commands", "[<variable>|<command>]")
{	
//	sipassert (_Commands != NULL);
