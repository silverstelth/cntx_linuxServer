...
//	delete [] buffer;
	...
	while (first != last)
	{
		...
		if (!CI18N::parseLabel(first, last, si.Identifier))
		{
			...
			sipwarning("DT: Fatal : In '%s', line %u: Invalid label after '%s'\n", 
				filename.c_str(), 
				line, 
				lastLabel.c_str());
			return false;
		...
		{
			uint32 line = countLine(text, first);
			sipwarning("DT: Fatal : In '%s', line %u: Invalid text value for label %s\n", 
				filename.c_str(), 
				line,
				lastLabel.c_str());
			return false;
		...
		if (specialCase)
		{
			...
			{
				uint32 line = countLine(text, first);
				sipwarning("DT: Fatal: In '%s' line %u: Invalid text2 value label %s\n", 
					filename.c_str(), 
					line,
					lastLabel.c_str());
				return false;
...
//			sipdebug("Generating hash for %s as %s", si.Identifier.c_str(), CI18N::hashToString(si.HashValue).c_str());
		}
		else
		{
//			sipdebug("Comment = [%s]", si.Comments.toString().c_str());
//			sipdebug("Retrieving hash for %s as %s", si.Identifier.c_str(), CI18N::hashToString(si.HashValue).c_str());
		}
		stringInfos.push_back(si);
	}
	...
	// check identifier uniqueness
	{
		...
		for (uint i=0; i<stringInfos.size(); ++i)
		{
			...
			if (it != unik.end())
			{
				sipwarning("DT: loadStringFile : identifier '%s' exist twice", stringInfos[i].Identifier.c_str() );
				error = true;
			}
			else
...
//			if (si.Comments.find(ucstring("// HASH_VALUE ")) == ucstring::npos)
			...
			str += ucstring("// INDEX ") + SIPBASE::toString("%u", first-strings.begin())+ nl;
			}
			str += si.Identifier + '\t';
...
//		sipdebug("Adding string [%s]", str.toString().c_str());
		diff += str;
	}
...
bool readPhraseFileFromString(ucstring const& doc, const std::string &filename, vector<TPhrase> &phrases, bool forceRehash)
{
	...
	while (first != last)
	{
		...
		if (!CI18N::parseLabel(first, last, phrase.Identifier))
		{
			...
			sipwarning("DT: In '%s' line %u: Error parsing phrase identifier after %s\n", 
				filename.c_str(),
				line,
				lastRead.c_str());
			return false;
		}
//		sipdebug("DT: parsing phrase '%s'", phrase.Identifier.c_str());
		lastRead = phrase.Identifier;
		CI18N::skipWhiteSpace(first, last, &phrase.Comments);
		if (!CI18N::parseMarkedString('(', ')', first, last, phrase.Parameters))
		{
			uint32 line = countLine(doc, first);
			sipwarning("DT: in '%s', line %u: Error parsing parameter list for phrase %s\n", 
				filename.c_str(),
				line,
		...
		{
			uint32 line = countLine(doc, first);
			sipwarning("DT: In '%s', line %u: Error parsing block opening '{' in phase %s\n", 
				filename.c_str(),
				line,
				phrase.Identifier.c_str());
			return false;
		...
		while (first != last && *first != '}')
		{
			...
			if (first == last)
			{
				sipwarning("DT: Found end of file in non closed block for phrase %s\n", phrase.Identifier.c_str());
				return false;
			}
			...
			while (first != last && *first == '(')
			{
				...
				{
					uint32 line = countLine(doc, first);
					sipwarning("DT: In '%s' line %u: Error parsing conditional expression in phrase %s, clause %u\n", 
						filename.c_str(),
						line,
						phrase.Identifier.c_str(), 
			...
			if (first == last)
			{
				sipwarning("DT: in '%s': Found end of file in non closed block for phrase %s\n", 
					filename.c_str(),
					phrase.Identifier.c_str());
				return false;
			}
			...
			{
				uint32 line = countLine(doc, first);
				sipwarning("DT: in '%s' line %u: Error reading text for clause %u (%s) in  phrase %s\n", 
					filename.c_str(),
					line,
					phrase.Clauses.size()+1, 
		...
		{
			uint32 line = countLine(doc, first);
			sipwarning("DT: in '%s' line %u: Missing block closing tag '}' in phrase %s\n", 
				filename.c_str(),
				line,
				phrase.Identifier.c_str());
			return false;
...
//		sipdebug("DT : storing phrase '%s'", phrase.Identifier.c_str());
		phrases.push_back(phrase);
	}
	...
	// check identifier uniqueness
	{
		...
		for (uint i=0; i<phrases.size(); ++i)
		{
			...
			if (it != unik.end())
			{
				sipwarning("DT: readPhraseFile : identifier '%s' exist twice", phrases[i].Identifier.c_str() );
				error = true;
			}
			else
...
bool readExcelSheet(const ucstring &str, TWorksheet &worksheet, bool checkUnique)
{
	...
	while ((pos = str.find(nl, lastPos)) != ucstring::npos)
	{
		...
		{
			strArray[pos]= 0;
//			sipdebug("Found line : [%s]", ucstring(&strArray[lastPos]).toString().c_str());
...
//	sipdebug("Found %u lines", lines.size());
	...
	for (i=0; i<lines.size(); ++i)
	{
		...
		if (newColCount != max(newColCount, numCells))
		{
			...
			sipdebug("At line %u, numCol changed to %u",
				i, newColCount);
		}
	}
	...
	for (i=0; i<lines.size(); ++i)
	{
		...
		for (; *first != 0; ++first)
		{
			if (*first == '\t')
			{
//				sipdebug("Found cell [%s]", cell.toString().c_str());
...
//		sipdebug("Found cell [%s]", cell.toString().c_str());
		...
		worksheet.setData(startLine + i, numCells, cell);
		numCells++;
		sipassertex(numCells<=newColCount, ("readExcelSheet: bad row format: at line %u, the row has %u cell, max is %u", i, numCells, newColCount));
//		sipdebug("Found %u cells in line %u", numCells, i);
	}
		...
		if (worksheet.size() > 0)
		{
			...
			if (nameCol < worksheet.ColCount )
			{
				...
				for (uint j=0; j<worksheet.size(); ++j)
				{
					...
					if (it != unik.end())
					{
						sipwarning("DT: readExcelSheet : identifier '%s' exist twice", worksheet.getData(j, nameCol).toString().c_str() );
						error = true;
					}
					else
