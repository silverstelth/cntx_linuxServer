...
// With SIP Memory Debug, use new
#ifndef SIP_USE_DEFAULT_MEMORY_MANAGER
# ifndef SIPBASE_HEAP_ALLOCATION_NDEBUG
#  define SIP_OV_USE_NEW_ALLOCATOR
# endif SIPBASE_HEAP_ALLOCATION_NDEBUG
...
public:
	...
	{
		destruct(0, _Size);
#ifndef SIP_OV_USE_NEW_ALLOCATOR
		free(_Ptr);
#else // SIP_OV_USE_NEW_ALLOCATOR
		if (_Ptr)
			delete [] (char*)_Ptr;
#endif // SIP_OV_USE_NEW_ALLOCATOR
		_Ptr= NULL;
		_Size= 0;
	}
	...
	/** Element accessor. no check is made on index. (No exception, no sipassert())
	 */
	 ...
	 *	sipassert if last is too big. copy(y, x, ...) where y>=x is valid, and nothing is copied.
	 */
