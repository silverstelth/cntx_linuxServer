...
#ifndef __BIT_MEM_STREAM_H__
#define __BIT_MEM_STREAM_H__
...
public:
	...
	void	addPoke( uint32 bitpos, uint32 size, TBMSSerialInfo::TSerialType type )
	{
		...
		if ( ! found )
		{
			sipwarning( "Missing reserve() corresponding to poke()" );
		}
		_DbgData->NextSymbol = NULL;
#endif
	}
	...
	std::string	getEventIdAtBitPos( uint32 bitpos, sint32 *eventId )
	{
		...
		if ( _DbgData->CurrentBrowsedItem < _DbgData->List.size() )
		{
			...
			//sipassert( bitpos < (*_List)[_CurrentBrowsedItem].BitPos ); // occurs if stream overflow
		}
#endif
		*eventId = -1;
		return std::string();
...
public:
	...
	void			resetBufPos()
	{
		...
		sipassert( ! ((!isReading()) && _Buffer.getBuffer().empty()) );
...
//		return (_BufPos - _Buffer.getPtr() + 1)*8 - _FreeBits;
	...
	void			displayStream( const char *title="", CLog *log = SIPBASE::DebugLog );
	 ...
	 */
	void			pokeBits( const SIPBASE::CBitSet& bitfield, uint bitpos );
	 ...
	 */
	void			readBits( SIPBASE::CBitSet& bitfield );

	/// Display the bits of the stream just before the specified bitpos (or current pos if -1)
	void			displayLastBits( sint nbits, sint bitpos=-1, SIPBASE::CLog *log=SIPBASE::DebugLog );
...
//		++_BufPos;
	...
	{
#ifdef SIP_DEBUG
//		sipassert( (!isReading()) && (!_Buffer.empty()) );
		sipassert( (!isReading()) && (!_Buffer.getBuffer().empty()) );
//		sipassert( _BufPos <= _Buffer.getPtr() + _Buffer.size() );
		sipassert( _Buffer.Pos <= _Buffer.getBuffer().size() );
#endif
...
void	displayBitStream( const CBitMemStream& msg, sint beginbitpos, sint endbitpos, SIPBASE::CLog *log=SIPBASE::DebugLog );
...
inline std::string CBMSDbgInfo::getEventLegendAtBitPos( CBitMemStream& bms, sint32 eventId )
{
	...
	{
		sipassert( eventId < (sint32)_DbgData->List.size() );
