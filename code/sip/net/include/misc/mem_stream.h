...
struct EMemStream : public SIPBASE::EStream
{
 ...
 * \seealso SIPNET::CMessage
 * \date 2000, 2002
 */
class CMemStream : public SIPBASE::IStream
{
public:

	/// Initialization constructor
	CMemStream( bool inputStream=false, bool stringmode=false, uint32 defaultcapacity=0 ) :
		SIPBASE::IStream( inputStream ), _StringMode( stringmode )
	{
	...
	void			poke( T value, sint32 pos )
	{
		if ( ! isReading() )
		{
			...
			sipassert( pokeBufPos + sizeof(T) <= pokeBufPos+_Buffer.Pos );
			*(T*)pokeBufPos = value;
		}
	}
...
// 2008/9/18 By RoMyongKuk : End
	...
	{
		//sipdebug( "MEMSTREAM: Writing %u-byte value in %p at pos %u", sizeof(value), this, _BufPos - _Buffer.getPtr() );
		increaseBufferIfNecessary (sizeof(T));
	...
	void			serial(T0 &a, T1 &b, T2 &c, T3 &d, T4 &e, T5 &f, T6 &g) 
	{ serial(a); serial(b); serial(c); serial(d); serial(e); serial(f); serial(g);}
	template<class T0,class T1,class T2,class T3,class T4,class T5,class T6,class T7>
	void			serial(T0 &a, T1 &b, T2 &c, T3 &d, T4 &e, T5 &f, T6 &g, T7 &h) 
	{ serial(a); serial(b); serial(c); serial(d); serial(e); serial(f); serial(g); serial(h);}
	template<class T0,class T1,class T2,class T3,class T4,class T5,class T6,class T7,class T8>
	void			serial(T0 &a, T1 &b, T2 &c, T3 &d, T4 &e, T5 &f, T6 &g, T7 &h, T8 &i) 
	{ serial(a); serial(b); serial(c); serial(d); serial(e); serial(f); serial(g); serial(h); serial(i);}
...
inline	void		CMemStream::serial(std::string &b) 
{
	if ( _StringMode )
	{
		...
		{
			serial(len);
			if (!checkStreamSize((uint)len))
			{
				b = "";
				return;
			}
			/*if (len>1000000)
				throw SIPBASE::EInvalidDataStream( "CMemStream/str: Trying to read a string of %u bytes", len );
			*/
			b.resize(len);
		}
		else
		{
			len= b.size();
			if (len>1000000)
				throw SIPBASE::EInvalidDataStream( "CMemStream/str: Trying to write a string of %u bytes", len );
			serial(len);
		}
...
// 2008/9/18 By RoMyongKuk : End
				/*
				if (len>1000000)
					throw SIPBASE::EInvalidDataStream( "CMemStream: Trying to read a string of %u bytes", len );
				*/
				b.resize(len);
				if (len > 0)
				{				
...
inline	void		CMemStream::serial(ucstring &b) 
{
	if ( _StringMode )
	{
		...
		if(isReading())
		{
			...
			/*
			if (len>1000000)
				throw SIPBASE::EInvalidDataStream( "CMemStream/str: Trying to read an ucstring of %u bytes", len );
			*/
			b.resize(len);
		}
		else
		{
			len= b.size();
			if (len>1000000)
				throw SIPBASE::EInvalidDataStream( "CMemStream/str: Trying to write an ucstring of %u bytes", len );
			serial(len);
		}
		// Read/Write the string.
		for(uint i=0;i!=len;++i)
