...
inline float frand(float minf, float maxf)
{
	float mod = maxf - minf;
	mod = frand(mod);
	return mod + minf;
...
#else
#	ifdef SIP_OS_MAC
	return !__inline_isnand(v) && !__inline_isinfd(v); //lci 09-11-28
#	else
...
/// ParamFormat() call that also allocates the target buffer.
...
/// ParamFormat() call that also allocates the target buffer (va_list version).
/*! \note Application must delete[] the buffer after use.
 *  \sa format()
 */
char *allocatedFormatVarArgs(const char *aFormat, va_list aVaList);
ucchar *allocatedFormatVarArgs(const ucchar *aFormat, va_list aVaList);

//! 문자?�을 복사?�는 ?�수. alen ??0?�면 모두 복사.
// * 주의: ?�로 기억구역???�당?��? ?�는??
char *copy(char *aStrOut, const char *aStrIn, sint alen = 0);
ucchar *copy(ucchar *aStrOut, const ucchar *aStrIn, sint alen = 0);

//! 문자?�을 복제?�는 ?�수
// * 주의: 기억구역??복제?�는 문자?�의 기억구역??주기?�에 ?�로 창조?��?�?
//				?�려지??문자?�의 주소�?마�?막에 delete?�시??
char *copy(const char *aString);
ucchar *copy(const ucchar *aString);

//! 문자?�을 ?�추가?�는 ?�수. alen ??0?�면 모두 ?�붓??
// * 주의: ?�로 기억구역???�당?��? ?�는??
char *cat(char *aStrOut, const char *aStrIn,  sint alen = 0);
ucchar *cat(ucchar *aStrOut, const ucchar *aStrIn,  sint alen = 0);
...
inline int wcsicmp(const ucchar *lhs, const ucchar *rhs) { 
#ifdef SIP_OS_MAC
	return strcasecmp((const char*) lhs, (const char*) rhs); //by lci-09-1128
#else
	return wcscasecmp(lhs, rhs); //by lci-09-0509
#endif
} 
	//inline int strnicmp(const ucchar *lhs, const ucchar *rhs, size_t n) { return wcsncasecmp(lhs, rhs, n); }
#endif

inline sint sipstricmp(const char *lhs, const char *rhs) { return stricmp(lhs, rhs); }
inline sint sipstricmp(const ucchar *lhs, const ucchar *rhs) { return wcsicmp(lhs, rhs); }

inline sint sipstricmp(const std::string &lhs, const std::string &rhs) { return stricmp(lhs.c_str(), rhs.c_str()); }
inline sint sipstricmp(const std::basic_string<ucchar> &lhs, const std::basic_string<ucchar> &rhs) { return wcsicmp(lhs.c_str(), rhs.c_str()); }

inline sint sipstricmp(const std::string &lhs, const char *rhs) { return stricmp(lhs.c_str(),rhs); }
inline sint sipstricmp(const std::basic_string<ucchar> &lhs, const ucchar *rhs) { return wcsicmp(lhs.c_str(),rhs); }

inline sint sipstricmp(const char *lhs, const std::string &rhs) { return stricmp(lhs,rhs.c_str()); }
inline sint sipstricmp(const ucchar *lhs, const std::basic_string<ucchar> &rhs) { return wcsicmp(lhs, rhs.c_str()); }

inline sint sipstrcmp(const char *lhs, const char *rhs) { return strcmp(lhs, rhs); }
inline sint sipstrcmp(const ucchar *lhs, const ucchar *rhs) { return wcscmp(lhs, rhs); }

inline sint sipstrcmp(const std::string &lhs, const std::string &rhs) { return strcmp(lhs.c_str(), rhs.c_str()); }
inline sint sipstrcmp(const std::basic_string<ucchar> &lhs, const std::basic_string<ucchar> &rhs) { return wcscmp(lhs.c_str(), rhs.c_str()); }

inline sint sipstrcmp(const std::string &lhs, const char *rhs) { return strcmp(lhs.c_str(),rhs); }
inline sint sipstrcmp(const std::basic_string<ucchar> &lhs, const ucchar *rhs) { return wcscmp(lhs.c_str(),rhs); }

inline sint sipstrcmp(const char *lhs, const std::string &rhs) { return strcmp(lhs,rhs.c_str()); }
inline sint sipstrcmp(const ucchar *lhs, const std::basic_string<ucchar> &rhs) { return wcscmp(lhs, rhs.c_str()); }
...
/**
 * Base class for all SIP exception.
...
protected:
	std::string _Reason;
	std::basic_string<ucchar> _ReasonW;

public:
	Exception();
	Exception(const std::string &reason);
	Exception(const std::basic_string<ucchar> &reason);
	Exception(const char   *format, ...);
	Exception(const ucchar *format, ...);
...
sint atoint(const char *aString, bool *aSuccess = NULL);
sint atoint(const ucchar *aString, bool *aSuccess = NULL);

/// Converts string to a float
float atofloat(const char *aString, bool *aSuccess = NULL);
float atofloat(const ucchar *aString, bool *aSuccess = NULL);

/// Converts string to a bool.
/*! String converted for case-insensitive way
*	TRUE, True, true => true
*	FALSE, False, false => false
*/
bool atob(const char *aString, bool *aSuccess = NULL);
bool atob(const ucchar *aString, bool *aSuccess = NULL);
 ...
 * So we can use stringstream operator << and >> with all SIP simple types (except for ucchar and ucstring)
 */
/*
#ifdef SIP_OS_WINDOWS
...
#define SIPBASE_ADD_BASIC_ISTREAM_OPERATOR(__type,__casttype) \
...
SIPBASE_ADD_BASIC_ISTREAM_OPERATOR(uint8, unsigned int);
SIPBASE_ADD_BASIC_ISTREAM_OPERATOR(sint8, signed int);
SIPBASE_ADD_BASIC_ISTREAM_OPERATOR(uint16, unsigned int);
SIPBASE_ADD_BASIC_ISTREAM_OPERATOR(sint16, signed int);
SIPBASE_ADD_BASIC_ISTREAM_OPERATOR(uint32, unsigned int);
SIPBASE_ADD_BASIC_ISTREAM_OPERATOR(sint32, signed int);
...
#ifdef SIP_OS_WINDOWS
inline int sipisprint(int c)
{
	if(c>255||c<0) return 0;
	return isprint(c);
}
#else
#define sipisprint isprint
#endif
...
struct CHashFunctionUInt64
{
	...
	size_t operator() (const uint64 &_Key) const
	{
		return (size_t)_Key;
	}

	bool operator()( const uint64& _Key1, const uint64& _Key2 ) const
	{
		return (_Key1 < _Key2);
	}
};
