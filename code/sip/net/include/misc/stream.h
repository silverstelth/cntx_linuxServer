...
#  define SIPBASE_BSWAP16(src)	(src) = (((src)>>8)&0xFF) | (((src)&0xFF)<<8)
#  ifdef SIP_OS_WINDOWS
#    define SIPBASE_BSWAP32(src) _asm mov eax,(src) _asm bswap eax _asm mov (src),eax
#  else
#    define SIPBASE_BSWAP32(src) (src) = (((src)>>24)&0xFF) | ((((src)>>16)&0xFF)<<8) | ((((src)>>8)&0xFF)<<16) | (((src)&0xFF)<<24)
#  endif
#  define SIPBASE_BSWAP64(src) (src) = (((src)>>56)&0xFF) | ((((src)>>48)&0xFF)<<8) | ((((src)>>40)&0xFF)<<16) | ((((src)>>32)&0xFF)<<24) | ((((src)>>24)&0xFF)<<32) | ((((src)>>16)&0xFF)<<40) | ((((src)>>8)&0xFF)<<48) | (((src)&0xFF)<<56)
...
struct EStream : public Exception
{
	...
	EStream( const std::basic_string<ucchar>& str ) : Exception( str ) {}

	EStream( const IStream &f );

	EStream( const IStream &f, const std::string& str );

	EStream( const IStream &f, const std::basic_string<ucchar>& str );

	virtual ~EStream() throw() {}

	// May Not be Filled...
	std::basic_string<ucchar> StreamName;
};
...
struct EInvalidDataStream : public EStream
{
	...
	EInvalidDataStream(const ucchar *msg, uint size);
};
...
struct EStreamOverflow : public EStream
{
	...
	EStreamOverflow(const ucchar *msg, uint size);
};
...
#define sipWriteSerial(_stream, _obj) 		\
	if ((_stream).isReading())			\
		throw SIPBASE::ENotOutputStream();	\
	(_stream).serial(SIPBASE::IStream::unconst(_obj));

#define sipWrite(_stream, _serialType, _obj)	\
	if ((_stream).isReading())			\
		throw SIPBASE::ENotOutputStream();	\
	(_stream)._serialType(SIPBASE::IStream::unconst(_obj));

#define sipReadSerial(_stream, _obj) 		\
	if (!(_stream).isReading())			\
		throw ENotInputStream();	\
	SIPBASE::IStream::unconst(_stream).serial(_obj);

#define sipRead(_stream, _serialType, _obj)	\
	if (!(_stream).isReading())			\
		throw SIPBASE::ENotInputStream();	\
	SIPBASE::IStream::unconst(_stream)._serialType(_obj);

// helper macro to serialize boolean encoded as 'bool foo : 1' (they can't be referenced)
#define sipSerialBitBool(_stream, _boolean) \
	{ \
...
#ifdef SIP_OS_UNIX
	void	serial(ucchar &b) { return serial((uint16&) b);}; //by LCI 09-05-09
#endif
	...
	virtual void	serial(std::basic_string<ucchar> &b) ;
	//@}
	...
	void			serialCont(HashMap<K, T, H> &cont) 			{serialMap(cont);}
	template<class K, class T>
	void			serialCont(std::multimap<K, T> &cont) 	{serialMultimap(cont);}

	/** \name standard STL containers serialisation.
	 * Thse variants suppose contained type is a SIP smart pointer.
...
protected:
	 ...
	 */
	bool				checkStreamSize(uint numBytes) const
	{
...
private:
	...
	HashMap<uint64, void*, CHashFunctionUInt64>						_IdMap;
	typedef HashMap<uint64, void*, CHashFunctionUInt64>::iterator		ItIdMap;
	typedef HashMap<uint64, void*, CHashFunctionUInt64>::value_type	ValueIdMap;
...
private:
	...
	/**
	 * STL map<>
	 * Support up to sint32 length containers. Container must contain SIP smart pointers.
	 *
	 * the object T must provide:
	 *	\li typedef iterator;		(providing operator++() and operator*())
	 *	\li typedef value_type;		(must be a std::pair<>)
...
#ifdef _UNICODE
#	define getStreamName getStreamNameW
#else
#	define getStreamName getStreamNameA
#endif
