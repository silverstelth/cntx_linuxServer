...
public:
    ...
    CRefCount() { crefs = 0; pinforef=static_cast<CPtrInfo*>(&NullPtrInfo); }
...
public:
	...
	mutable	CPtrInfo	*pinforef;	// The ref ptr for RefPtr use.
	
	/// operator= must NOT copy crefs/pinfo!!
	CRefCount &operator=(const CRefCount &) {return *this;}
	/// copy cons must NOT copy crefs/pinfo!!
	CRefCount(const CRefCount &) {crefs = 0; pinforef=static_cast<CPtrInfo*>(&NullPtrInfo);}
};


template <class T>
class	COneBaseRefCount : public CRefCount
{
	T	*ptr;
public:
	COneBaseRefCount()	{ptr=NULL;}
	COneBaseRefCount(T	*_p)	{ptr=_p;}
	virtual ~COneBaseRefCount() { delete ptr; }
};
...
public:
	...
	void serialPtr(SIPBASE::IStream &f) throw(SIPBASE::EStream ) 
	{
		T*	obj= NULL;
		if(f.isReading())
		{
	...
	void serialPolyPtr(SIPBASE::IStream &f) throw(SIPBASE::EStream ) 
	{ 
		T*	obj= NULL;
		if(f.isReading())
		{
...
public:
	 ...
	 *   This Debug feature depends on SIP_DEBUG_PTR definition (change it as your convenience).
	 *   These are only debug classes and are not design to use information for your code comportment
	 *  as they are made to desappear in final version (SIP_DEBUG_PTR undefined),
	 *  so don't use method calls outside SIP_DEBUG_PTR scope.
	 *
	 * How to:
...
public:
	...
	virtual	~CDbgRefCount()	
	{
		if (_DbgCRefs!=0 || _DbgCCstRefs!=0)
		{
			...
			sipassert(_DbgCRefs==0);
			sipassert(_DbgCCstRefs==0);
		}
	}
	...
	{
		if (_CheckOn)
			sipassert(_DbgCRefs<_MaxRef);
		++_DbgCRefs;
		// Linked list management
		sipassert(_FirstReference!=&ptr);
	...
	{
		if (_CheckOn)
			sipassert(_DbgCCstRefs<_MaxRef);
		++_DbgCCstRefs;
	}
...
public:
...
#endif
	{
		Ptr = const_cast<T*>(SIPBASE::type_cast<const T*>(p));
	#ifdef SIP_DEBUG_PTR
		if (Ptr)
		{
	...
	{
		return SIPBASE::type_cast<W*>(Ptr);
	}
	...
	CDbgPtr<T>& operator=(const W* p)
	{
		...
		if (p)
			newRef = static_cast<CDbgRefCount<T>*>(const_cast<T*>(SIPBASE::type_cast<const T*>(p)));
		if (oldRef!=newRef)
		{
	...
	*/
	...
	{
		return Ptr==SIPBASE::type_cast<const T*>(p);
	}
	template <class W>
	bool operator !=(const W* p) const
	{
