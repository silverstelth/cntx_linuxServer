...
#ifndef	__EID_TRANSLATOR_H__
#define	__EID_TRANSLATOR_H__
...
{
	SIPBASE_SAFE_SINGLETON_DECL_PTR(CEntityIdTranslator);
public:
	...
	typedef std::map<SIPBASE::CEntityId, CEntity>	TEntityCont;
	...
	const ucstring		&getByEntity (const SIPBASE::CEntityId &eid);
...
private:
	...
	void				getByUser (uint32 uid, std::vector<SIPBASE::CEntityId> &res);
	void				getByUser (const std::string &userName, std::vector<SIPBASE::CEntityId> &res, bool exact=true);
	
	void				getByEntity (const ucstring &entityName, std::vector<SIPBASE::CEntityId> &res, bool exact);
	...
	bool isValidEntityName (const ucstring &entityName, SIPBASE::CLog *log = SIPBASE::InfoLog );


	/// The container for all entity in the translator
	TEntityCont			RegisteredEntities;

	typedef std::map<ucstring, SIPBASE::CEntityId>	TNameIndexCont;
	...
	SIPBASE_CATEGORISED_COMMAND_FRIEND(sip,findEIdByUser);
	SIPBASE_CATEGORISED_COMMAND_FRIEND(sip,findEIdByEntity);
	SIPBASE_CATEGORISED_COMMAND_FRIEND(sip,entityNameValid);
	SIPBASE_CATEGORISED_COMMAND_FRIEND(sip,playerInfo);
};

}

#endif // __EID_TRANSLATOR_H__
