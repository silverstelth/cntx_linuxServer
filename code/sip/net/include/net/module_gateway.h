...
namespace SIPNET
{
	 ...
	 *	There are available transport for SIP Layer 3 in server
	 *	of client mode.
	 *
	 ...
	 */
	class IModuleGateway : public SIPBASE::CRefCount
	{
	public:
		// Exception classes ===================================

		/// The gateway is already open while trying to open it.
		class EGatewayAlreadyOpen : public SIPBASE::Exception
		{
		};

		/// When trying to open the gateway server, the TCP port is already in use
		class EGatewayPortInUse : public SIPBASE::Exception
		{
		};
		
		/// The gateway is not open while trying to close it.
		class EGatewayNotOpen : public SIPBASE::Exception
		{
		};

		/// A gateway is not connected while trying to communicate with
		class EGatewayNotConnected : public SIPBASE::Exception
		{
		};

		/// Firewall mode is activated but there is already open route !
		class EGatewayFirewallBreak : public SIPBASE::Exception
		{
		};
...
//		virtual void onReceiveModuleMessage(TModuleGatewayProxyPtr &senderGateway, TModuleMessagePtr &message) =0;
		 ...
		 */
		virtual void sendModuleMessage(IModuleProxy *senderProxy, IModuleProxy *addresseeProxy, const SIPNET::CMessage &message) =0;
	 ...
	 *	
	 *	At time of writing, SIP come with 2 transport : one based on layer 3 client, and one
	...
	public:
		...
		class EInvalidCommand : public SIPBASE::Exception
		{
		public:
			EInvalidCommand() {}
		...
		class ETransportError : public SIPBASE::Exception
		{
		public:
		...
		/// Constructor param needed by the factory (see misc/factory.h)
		struct TCtorParam
		{
		...
		virtual void dump(SIPBASE::CLog &log) const =0;
	};
...
//		typedef std::map<TModuleId, TModuleId>	TForeignToLocalIdx;
		typedef SIPBASE::CTwinMap<TModuleId, TModuleId>	TForeignToLocalIdx;
...
//		std::set<TModuleId>		PendingUndisclosure;
 ...
 #define SIPNET_AUTO_DELTE_ASSERT	CAutoAssertSetter __autoDeleteAssert(static_cast<const CGatewayRoute&>(*this))
#else
 #define SIPNET_AUTO_DELTE_ASSERT
#endif
