...
#ifndef __UNIFIED_NETWORK_H__
#define __UNIFIED_NETWORK_H__
...
public:
	...
	{
		return SIPBASE::toString("%u", _ServiceId);
	}
 ...
 *	This service can be used in a SIPBASE::CEntityId.
 ...
 * entire data at a time. The SIP Network Layer 5 will silently continue streaming up the remaining
 ...
 * \date 2002-2004
 */
class CUnifiedNetwork : public SIPBASE::ICommandsHandler
{
...
//	static CUnifiedNetwork *getInstance ();
	...
	void	closeWSConnection (const std::string &serviceName);
	 ...
	 */
	void	addDefaultNetwork (const std::string &defnet) { sipinfo ("HNETL5: Add default network '%s'", defnet.c_str()); _DefaultNetwork.push_back (defnet); }
	...
	CCallbackNetBase	*getNetBase(const std::string &name, TTcpSockId &host, uint8 nid=0xFF);

	/// Gets the CCallbackNetBase of the service
	CCallbackNetBase	*getNetBase(TServiceId sid, TTcpSockId &host, uint8 nid=0xFF);
	...
	/// Gets the message number of processing per millisecond
	uint32				getProcMessagePerMS ();

	/// Find a callback in the array
	TUnifiedMsgCallback findCallback (const MsgNameType &callbackName);
	...
	void				displayInternalTables (SIPBASE::CLog *log = SIPBASE::InfoLog);

	void				displayUnifiedConnection (TServiceId sid, SIPBASE::CLog *log = SIPBASE::InfoLog)
	{
		getUnifiedConnection(sid)->display(false, log);
	}
	
	const CInetAddress&	listenAddress() const { return _CbServer->listenAddress(); }

	void				SetLogRecvPacketFlag(bool b)
	{
		if (_CbServer)
			_CbServer->SetLogRecvPacketFlag(b);
	}

private:

	/// A map of service ids, referred by a service name
	struct TNameMappedConnection : public HashMultiMap<std::string, TServiceId> {};

	/// A map of callbacks, refered by message name
	typedef std::map<MsgNameType, TUnifiedMsgCallback>			TMsgMappedCallback;
	...
	typedef HashMap<std::string, std::list<TCallbackArgItem> >	TNameMappedCallback;
	...
	public:
		...
		struct TConnection
		{
			...
			TConnection(CCallbackNetBase *cbnb, TTcpSockId hi) : IsServerConnection(true), CbNetBase(cbnb), HostId(hi) { }

			void setAppId (uint64 appid) { CbNetBase->getTcpSockId (HostId)->setAppId (appid); }
			uint64 getAppId () { return CbNetBase->getTcpSockId (HostId)->appId (); }
		...
		void display (bool full, SIPBASE::CLog *log = SIPBASE::InfoLog);
		...
		void setupNetworkAssociation (const std::vector<uint32> &networkAssociations, const std::vector<std::string> &defaultNetwork)
		{
			for (uint i = 0; i < networkAssociations.size (); i++)
			{
				...
				for (j = 0; j < ExtAddress.size (); j++)
				{
					if (ExtAddress[j].internalNetAddress() == networkAssociations[i])
					{
						...
						sipinfo ("HNETL5: nid %u will be use connection %u", i, j);
						break;
					}
				}
				if (j == ExtAddress.size ())
				{
					sipinfo ("HNETL5: nid %u is not found", i);
				}
			}
			...
			for (j = 0; j < defaultNetwork.size (); j++)
			{
				...
				if (pos != std::string::npos && pos == 0 && ServiceName.size() == defaultNetwork[j].size ()-1)
				{
					...
					sipinfo ("HNETL5: default network for '%s' will be nid %hu and connection id %hu", ServiceName.c_str(), (uint16)nid, (uint16)DefaultNetwork);
					break;
				}
			}
			if (j == defaultNetwork.size ())
			{
				...
				if (defaultNetwork.size () > 0)
					sipwarning ("HNETL5: default network not found in the array, will use connection id %hu", (uint16)DefaultNetwork);
			}
		}
	};

	SIPBASE_COMMAND_HANDLER_TABLE_BEGIN(CUnifiedNetwork)
		SIPBASE_COMMAND_HANDLER_ADD(CUnifiedNetwork, addService, "Add a service in the unified network", "<serviceName> ( address=<address:port> [sid=<serviceId>] [sendId] [external] [autoRetry] )")
	SIPBASE_COMMAND_HANDLER_TABLE_END

	SIPBASE_CLASS_COMMAND_DECL(addService);
...
protected:
	...
	void				sleepUntilDataAvailable( SIPBASE::TTime msecMax );
#endif
...
//	static CUnifiedNetwork						*_Instance;
	...
	friend struct sip_isServiceLocalClass;
	friend struct sip_l5CallbackClass;
	friend struct sip_l5QueuesStatsClass;
};
