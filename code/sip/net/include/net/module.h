...
namespace SIPNET
{
	...
	public:
		...
		class EModuleAlreadyPluggedHere : public SIPBASE::Exception
		{
		};

		/// An operation invocation has failed (mostly because of lost server module)
		class EInvokeFailed : public SIPBASE::Exception
		{
		};

		/// An operation invocation has failed because of a bad return type from servant
		class EInvokeBadReturn : public SIPBASE::Exception
		{
		};
		...
		//@{
		/// A Sip layer 5 service has started.
		virtual void				onServiceUp(const std::string &serviceName, SIPNET::TServiceId serviceId) =0;
		/// A Sip layer 5 service has stopped.
		virtual void				onServiceDown(const std::string &serviceName, SIPNET::TServiceId serviceId) = 0;
...
//		virtual void				onModuleSecurityChange(IModuleProxy *moduleProxy) =0;
		 ...
		 */
		virtual void		invokeModuleOperation(IModuleProxy *destModule, const SIPNET::CMessage &opMsg, SIPNET::CMessage &resultMsg) 
			throw (EInvokeFailed)
			=0;
	...
	struct TSecurityData
	{
		...
		virtual void serial(SIPBASE::CMemStream &s) =0;
...
//	SIPBASE_REGISTER_OBJECT(TSecurityData, TUnknownSecurityData, uint8, 0xff);
	 ...
	 */
	class IModuleProxy : public SIPBASE::CRefCount
	{
	public:
		 ...
		 *	the message.
		 */
		virtual void		sendModuleMessage(IModule *senderModule, const SIPNET::CMessage &message)
	...
	class CModuleTask : public SIPBASE::CCoTask
	{
		bool	_FailInvoke;
	protected:
	...
	private:
		...
		void run()
		{
			...
			{
				sipwarning("In module task '%s', exception '%e' thrown", typeid(this).name(), e.what());
			}
			catch (...)
			{
				sipwarning("In module task '%s', unknown exception thrown", typeid(this).name());
			}
...
#define	SIPNET_START_MODULE_TASK(className, methodName) \
	{ \
	 ...
	 *	the module manager.
	 */
	class IModuleFactory : public SIPBASE::CRefCount
	{
	protected:
...
#define SIPNET_REGISTER_MODULE_FACTORY(moduleClassName, registrationName) \
	class moduleClassName##Factory : public SIPNET::CModuleFactory<moduleClassName> \
	{ \
	public: \
		...
		moduleClassName##Factory() \
			: SIPNET::CModuleFactory<moduleClassName>(theModuleClassName()) \
		{} \
	};\
	SIPBASE_REGISTER_OBJECT_INDIRECT(SIPNET::IModuleFactory, moduleClassName##Factory, std::string, registrationName)


//#define SIPNET_MAKE_MODULE_FACTORY_TYPENAME(moduleClassName) moduleClassName##Factory

#define SIPNET_GET_MODULE_FACTORY(moduleClassName)	moduleClassName##Factory
	 ...
	 */
	class CModuleBase : public IModule, public SIPBASE::ICommandsHandler
	{
		...
		const SIPNET::CMessage	*_CurrentMessage;
	...
	protected:
		...
		void				invokeModuleOperation(IModuleProxy *destModule, const SIPNET::CMessage &opMsg, SIPNET::CMessage &resultMsg) throw (EInvokeFailed);
		...
		SIPBASE_COMMAND_HANDLER_TABLE_BEGIN(CModuleBase)
			SIPBASE_COMMAND_HANDLER_ADD(CModuleBase, dump, "display informations about module instance status", "no args")
			SIPBASE_COMMAND_HANDLER_ADD(CModuleBase, plug, "plug the module in a module socket", "<socket_name>")
			SIPBASE_COMMAND_HANDLER_ADD(CModuleBase, unplug, "unplug the module out of a module socket", "<socket_name>")
			SIPBASE_COMMAND_HANDLER_ADD(CModuleBase, sendPing, "send a ping message to another module using the first available route", "<addresseeModuleName>")
		SIPBASE_COMMAND_HANDLER_TABLE_END

		SIPBASE_CLASS_COMMAND_DECL(dump);
		SIPBASE_CLASS_COMMAND_DECL(plug);
		SIPBASE_CLASS_COMMAND_DECL(unplug);
		SIPBASE_CLASS_COMMAND_DECL(sendPing);
	};
	...
	class CModuleProxy : public IModuleProxy
	{
		...
		SIPBASE::TStringId	_ModuleClassName;
		/// The  fully qualified module name.
		SIPBASE::TStringId	_FullyQualifiedModuleName;
		/// The module manifest
	...
	class TBroadcastModuleMessage
	{
		...
		void sendMessage(IModule *sender, const PtrContainer &addresseeProxies, const SIPNET::CMessage &message)
		{
