...
#ifndef __PACS_CLIENT_H__
#define __PACS_CLIENT_H__
...
#include "pacs/u_move_primitive.h"
#include "pacs/u_collision_desc.h"

#define SIPNET_PACS_PROTOCOL_VERSION 1
...
/**
 * Client side of Pacs Service. Allows to use PACS functionnality by the network.
 *
 * \date 2001
 */
class CPacsClient
...
public:
	...
	void	initMessage ()
	{
		...
		_Message.serialCheck ((uint32)SIPNET_PACS_PROTOCOL_VERSION);
	}
	...
	/**
	  ...
	  */
	void testMove (NLPACS::UMovePrimitive::TUserData id, const SIPBASE::CVectorD& speed, double deltaTime)
	{
		// Append to the current message
		std::string name="TS";
		bool nlTrue=true;
		_Message.serial (nlTrue, name, id, const_cast<SIPBASE::CVectorD&> (speed), deltaTime);
	}
	...
	/**
	  ...
	  */
	void globalMove (const SIPBASE::CVectorD& position)
	{
		// Append to the current message
		std::string name="GM";
		bool nlTrue=true;
		_Message.serial (nlTrue, name, const_cast<SIPBASE::CVectorD&> (position));
	}
	...
	/**
	  ...
	  * \param position is the new position for the primitive on the service.
	  */
	void relativeMove (const SIPBASE::CVectorD& speed)
	{
		// Append to the current message
		std::string name="RM";
		bool nlTrue=true;
		_Message.serial (nlTrue, name, const_cast<SIPBASE::CVectorD&> (speed));
	}
...
protected:
	...
	/**
	  ...
	  */
	virtual void	getPositionSpeedCallback (NLPACS::UMovePrimitive::TUserData id, const SIPBASE::CVectorD &position, const SIPBASE::CVectorD &speed)
	{}
...
static void cbPacsAnswer (CMessage &msgin, TSockId from, CCallbackNetBase &netbase)
{
	...
	// Check stream
	msgin.serialCheck ((uint32)SIPNET_PACS_PROTOCOL_VERSION);
	...
	while (again)
	{
		...
		else if (subMessage=="PS")
		{
			// Read the primitive id and test result
			NLPACS::UMovePrimitive::TUserData id;
			SIPBASE::CVectorD position;
			SIPBASE::CVectorD speed;
			...
			SIPBASE::sipError ("Pacs client: unkown sub message string");
