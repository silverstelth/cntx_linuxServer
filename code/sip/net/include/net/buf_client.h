...
#ifndef __BUF_CLIENT_H__
#define __BUF_CLIENT_H__
 ...
 */
class CClientReceiveTask : public SIPBASE::IRunnable
{
public:

	/// Constructor (increments the reference to the object pointed to by the smart pointer sockid)
	CClientReceiveTask( CBufClient *client, CNonBlockingBufSock *bufsock, CBufUdpClient *bufUdp ) : NbLoop(0), _Client(client), _NBBufSock(bufsock), _ExitRequired(false), _BufUdpSock(bufUdp) {} // CHANGED: non-blocking client connection
	...
	TTcpSockId					sockId() { return (TTcpSockId)_NBBufSock; }
...
private:
	...
	CBufUdpClient				*_BufUdpSock;
};

class CClientReceiveTaskUdp : public SIPBASE::IRunnable
{
public:

	/// Constructor (increments the reference to the object pointed to by the smart pointer sockid)
	CClientReceiveTaskUdp( CBufClient *client, CNonBlockingBufSock *bufsock, CBufUdpClient *bufUdp ) : NbLoop(0), _Client(client), _NBBufSock(bufsock), _BufUdpSock(bufUdp) {} // CHANGED: non-blocking client connection
...
public:
	 ...
	 */
	CBufClient( bool nodelay=true, bool replaymode=false, bool initPipeForDataAvailable=true, bool bHasUdp=false, bool bNoRecieve = false );
	...
	// do aborted shutdown and closing
	void	forceClose( bool quick=false );
	...
	void	send( const SIPBASE::CMemStream& buffer, uint8 bySendType = REL_GOOD_ORDER );
	...
	void	receive( SIPBASE::CMemStream& buffer );
	...
	{ 
		uint32 total = _BufSock->SendFifo.size();

		return total;
	}

	void displaySendQueueStat (SIPBASE::CLog *log = SIPBASE::InfoLog)
	{
		_BufSock->SendFifo.displayStats(log);
	}
	
	void displayThreadStat (SIPBASE::CLog *log);
	...
	*/
	...
	TTcpSockId	id()	const { return _BufSock; /*_RecvTask->sockId();*/ }
	CBufUdpClient*	getUdpClient() const { return _BufUdpSock; }

	bool	NoRecieve() { return _NoRecieve; };
...
protected:
	...
	*/
	uint32				_TcpIDOfInServer;
	bool				_RecvTcpConfirm;
	bool				_FinishTcpConfirm;
private:

	/// Receive task
	CClientReceiveTask	*_RecvTask;
	/// Receive thread
	SIPBASE::IThread		*_RecvThread;

	/// Receive task
	CClientReceiveTaskUdp	*_RecvTaskUdp;
	/// Receive thread
	SIPBASE::IThread		*_RecvThreadUdp;
};
