...
TUnifiedCallbackItem WorldCallbackArray[] =
{
	...
	// ?§‰æ©Á£äÂïä ÁªµÊ±óÂ¢®Èùõ???§‰æ©Á™çÁúã‰ø?
...
void init_WorldService()
{
	// Connect to database
	{
		...
		// DBÈæãÂÖç???¥Ëæë???°Áò§?ªÂíØ??‰∏??òÂ§∏??‰πêÊâÅ??∑©‰ø?4??1???êÊ≤•?Ñ‰øÉ.
...
static void	DBCB_DBGetOwnManageRoom(int nQueryResult, uint32 argn, void *argv[], CMemStream * resSet)
{
	...
	{
		// Packet??Ëæ®Êçû???àÂÖ¨ Ëæ®Áª¢Ëæ??ãÊâºËæ??äËæ∞‰ø?
...
{
	// «‡ªÁ: π∞¡÷±‚, ≤…µÂ∏Æ±‚, ±ÊªÛµÓµÂ∏Æ±‚, π∞∞Ì±‚∏‘¿Ã¡÷±‚, √µºˆ¿«Ωƒ, π∞∞«≈¬øÏ±‚, «œ¥√¿«Ωƒ, ∂•¿«Ωƒ
	// Family_Exp_Type_Visitday, Family_Exp_Type_Visitroom - ¿Ã∞« «œ¡ˆ æ ¥¬∞Õ¿∏∑Œ «œ¿⁄!! (Family_Exp_Type_Visitroom√≥∏Æø° πÆ¡¶¿÷¿Ω)
	...
	uint8	FamilyExpType;
	FamilyExpType = Family_Exp_Type_Flower_Water;	// π∞¡÷±‚ 1π¯
	if (Param & 1)	// π∞¡÷±‚∞° ¡¯«‡µ«ø¥¿∏∏È
	{
		if (today != FamilyInfo.m_FamilyExpHis[FamilyExpType].Day)
		{
	...
	FamilyExpType = Family_Exp_Type_XingLi;	// ¿ŒªÁµÂ∏Æ±‚ 1π¯
	if (mstFamilyExp[FamilyExpType].MaxCount > FamilyInfo.m_FamilyExpHis[FamilyExpType].Count)
	{
		if (today != FamilyInfo.m_FamilyExpHis[FamilyExpType].Day)
		{
	...
	FamilyExpType = Family_Exp_Type_Memo;	// ±ÊªÛµÓµÂ∏Æ±‚ 1π¯
	if (Param & 2)	// ±ÊªÛµÓµÂ∏Æ±‚∞° ¡¯«‡µ«ø¥¿∏∏È
	{
		if (today != FamilyInfo.m_FamilyExpHis[FamilyExpType].Day)
		{
			FamilyInfo.m_FamilyExpHis[FamilyExpType].Day = today;
	...
	FamilyExpType = Family_Exp_Type_Fish;	// π∞∞Ì±‚∏‘¿Ã¡÷±‚ CountPerDayπ¯
	if (YangyuItemID != 0)
	{
		if (today != FamilyInfo.m_FamilyExpHis[FamilyExpType].Day)
		{
	...
	FamilyExpType = Family_Exp_Type_Jisi_One;	// √µºˆ¿«Ωƒ CountPerDayπ¯
	...
	FamilyExpType = Family_Exp_Type_Xianbao;	// π∞∞«≈¬øÏ±‚ CountPerDayπ¯
	...
	FamilyExpType = Family_Exp_Type_ReligionYiSiMaster;	// «œ¥√¿«Ωƒ CountPerDayπ¯
	if (TianItemID != 0)
	{
		if (today != FamilyInfo.m_FamilyExpHis[FamilyExpType].Day)
		{
	...
	FamilyExpType = Family_Exp_Type_ReligionYiSiMaster;	// ∂•¿«Ωƒ CountPerDayπ¯
	if (DiItemID != 0)
	{
		if (today != FamilyInfo.m_FamilyExpHis[FamilyExpType].Day)
		{
...
// øπæ‡«‡ªÁø° ¿««— ∞Ê«Ëƒ°¡ı∞° ≈Î¡ˆ LS->WS->MS->OROOM ([d:FID][d:YishiTimeMin][d:Autoplay3ID][d:RoomID][u:RoomName][d:CountPerDay][d:YishiTaocanItemID][d:XianbaoTaocanItemID][d:YangyuItemID][d:ItemID_Tian][d:ItemID_Di][d:Param])
...
// øπæ‡«‡ªÁø° ¿««— ∞Ê«Ëƒ°¡ı∞° ≈Î¡ˆ LS->OwnWS->MS->OROOM ([d:FID][d:YishiTimeMin][d:Autoplay3ID][d:RoomID][u:RoomName][d:CountPerDay][d:YishiTaocanItemID][d:XianbaoTaocanItemID][d:YangyuItemID][d:ItemID_Tian][d:ItemID_Di][d:Param])
